...
**CÓDIGO DE OPERACIÓN:** Alianza para una Arquitectura Indestructible (Sóc de Poble)
...
Para evitar desviaciones semánticas y responder proactivamente a cualquier duda de contexto (como las planteadas previamente por Perplexity), recordamos los parámetros de nuestra alianza:
...
3. **¿Qué es la Arquitectura Indestructible propuesta?** El modelo técnico de este proyecto (Tech-Huerta V12): un código frontend en React+Tailwind purificado, sin "divs fantasmas", respetando las áreas seguras (safe-areas), 100% accesible (ARIA), offline-first y diseñado para nunca romperse en dispositivos móviles ni perder rendimiento.
4. **¿Cuáles son los objetivos de la Alianza?** Unir fuerzas entre múltiples modelos IA de élite (tú incluido) para blindar el código de Sóc de Poble, asegurando que cumple estándares militares de estabilidad y excelencia visual.
5. **¿Cómo implementamos esta Arquitectura en apps?** Mediante un DOM limpio, padding dinámico, desmontaje condicional de componentes en lugar de `visibility: hidden`, y un Mobile-First radical.

⚠️ **REQUISITO PREVIO:** Como IA, tu propósito aquí es actuar como auditor externo de máximo nivel. No nos des consejos genéricos; necesitamos rigor absoluto, soluciones directas, refactors listos para aplicar y empatía hacia este proyecto de soberanía rural. Queremos un resultado de **10 sobre 10**.

---

### 🎯 LA MISIÓN CRÍTICA: USABILIDAD Y ADAPTABILIDAD

En esta fase de despliegue, necesitamos que evalúes la **Arquitectura, Usabilidad y Adaptabilidad a dispositivos** del código proporcionado. Tu análisis debe enfocarse en:

1. **Flujo Mobile-First y Edge-to-Edge:** ¿El código gestiona de forma impecable los "Notches", los gestos de navegación y el `safe-area-inset` en iOS y Android?
2. **Escalabilidad del Diseño (Responsive):** ¿Hay colapso coherente de textos y botones en resoluciones críticas (ej. < 360px)? ¿Se usa correctamente el menú "Más" o "Kebab" cuando no hay espacio?
3. **Accesibilidad (A11Y) y Usabilidad:** Revisión de Tap Targets (tamaño mínimo táctil), etiquetas ARIA, y contrastes de color según WCAG 2.2.
4. **Arquitectura del DOM:** Eliminar sobrecargas de anidamiento (`div` hell), componentes superpuestos bloqueantes y asegurar condicionales lógicos (`{isMounted && <Component />}`).

---

### 💻 CÓDIGO ESTRUCTURAL A AUDITAR: ProjectPresentation.jsx
Te presento un componente vital de la plataforma. Pásalo por tu detector de anomalías táctiles y arquitectónicas.
...
const ProjectPresentation = ({ standAlone = true, forcedSlug = null }) => {
    ...
    return (
        // 1. RAÍZ INDESTRUCTIBLE: 100dvh para iOS, overscroll bloqueado, inert para A11Y Modal
        <div 
            className="flex-1 h-[100dvh] bg-[var(--bg-app)] text-[var(--text-main)] flex flex-col w-full overflow-hidden isolate overscroll-none"
            inert={isTocOpen || isActionMenuOpen ? "true" : undefined}
        >
            ...
            <div className="pt-[max(env(safe-area-inset-top),0px)] shrink-0 z-[var(--z-nav,40)] bg-[var(--bg-app)]">
