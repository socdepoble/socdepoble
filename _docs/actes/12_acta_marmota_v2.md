...
- **Components del Sistema de Disseny (Solutia):** A `src/components/design-system/sections/LegacySections.jsx` tenim els fonaments del disseny. Vam inserir amb èxit les targetes referents (Targeta de Botiga per la Samarreta Edició Gris, i Targeta Vídeo Projecte Sóc de Poble) a la Secció 24. A més, hem exterminat TOTS els errors del *Linter* sobre estils React *inline* il·legals. Ara és terreny 100% Tailwind.
...
**QUÈ HAS DE FER ARA:**
1. **Confirma que entens la missió.** Escriu directament al Mestre demostrant que has entès el punt d'ahir (resolem bug scroll, targetes Solutia rescatades, i linter CSS arreglat). 
2. **Presentació Nanovisor:** Posa la teva capçalera de mètriques i estalvia't explicacions genèriques. Mostra *Trellat*.
