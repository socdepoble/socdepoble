...
import { docExtractionService } from "../core/services/docExtractionService";
 ...
 */
const HerenciaManager = ({ onBack, predefinedUser }) => {
...
Atum! El sistema bategua amb el trellat de la terra.`;
  ...
  return (
    <TramitBaseTemplate
      title="Gestió d'Herència"
      description="Protocol notarial i deutes Archon."
      icon={Landmark}
      status="completed"
      onBack={onBack}
      predefinedUser={predefinedUser}
      tabs={[
        { id: 'history', label: "Històric d'Expedients", icon: Clock },
        { id: 'new', label: "Nou Expedient", icon: Plus }
      ]}
      activeTab={activeTab}
      onTabChange={setActiveTab}
    >
      <div className={`herencia-manager animate-in transition-all duration-500 ${iaiaSidebarOpen ? 'sidebar-open' : ''} !h-auto w-full flex flex-col bg-[#111] relative`}>
        
        {/* TAB NAVIGATION REMOVED, DELEGATED TO TRAMIT BASETEMPLATE */}

        {activeTab === 'history' && (
           <div className="p-6 md:p-10 space-y-6">
              <div className="p-6 rounded-[32px] bg-white/5 border border-white/10 flex items-center justify-between hover:bg-white/10 cursor-pointer transition-all" onClick={() => setActiveTab('new')}>
                 <div>
                    <h3 className="text-white font-bold mb-1">Herència - EXP: {sumaData?.expediente || '028468'}</h3>
                    <p className="text-gray-400 text-xs">Herminio • Fase Liquidació (SUMA)</p>
                 </div>
                 <div className="text-right">
                    <span className="inline-block px-3 py-1 bg-fuchsia-500/20 text-fuchsia-400 rounded-full text-[10px] font-bold tracking-wider uppercase border border-fuchsia-500/30">Bategant</span>
                    <p className="text-gray-300 font-bold mt-2">{sumaData?.total_pagar?.toFixed(2) || '226.69'} € Deute</p>
                 </div>
              </div>
           </div>
        )}
