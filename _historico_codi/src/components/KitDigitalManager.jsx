 ...
 */
const KitDigitalManager = ({ onBack, predefinedUser }) => {
...
Atum! El sistema bategua amb el certificat segellat per a Sollutia.`;
  ...
  return (
    <TramitBaseTemplate
      title="Kit Digital: Acord Sollutia"
      description="Gestió sobirana de la subvenció Kit Digital amb assistència de l'Archon MarIA."
      icon={Computer}
      status="completed"
      onBack={onBack}
      predefinedUser={predefinedUser}
      tabs={[
        { id: 'history', label: "Històric d'Acords", icon: Clock },
        { id: 'new', label: "Nou Acord", icon: Plus }
      ]}
      activeTab={activeTab}
    ...
    >
      <div className="w-full h-full flex flex-col relative bg-[#111]">
        ...
        {activeTab === 'history' && (
           <div className="p-6 md:p-10 space-y-6">
              <div className="p-6 rounded-[32px] bg-white/5 border border-white/10 flex items-center justify-between hover:bg-white/10 cursor-pointer transition-all" onClick={() => setActiveTab('new')}>
                 <div>
                    <h3 className="text-white font-bold mb-1">KD/0001622986 - Plan SEO Control</h3>
                    <p className="text-gray-400 text-xs">SOLLUTIA, S.L. • 15/02/2026</p>
                 </div>
                 <div className="text-right">
                    <span className="inline-block px-3 py-1 bg-green-500/20 text-green-400 rounded-full text-[10px] font-bold tracking-wider uppercase border border-green-500/30">Signat</span>
                    <p className="text-gray-300 font-bold mt-2">1.000,00 €</p>
                 </div>
              </div>
           </div>
        )}
        ...
        {activeTab === 'new' && (
      <div className="kit-digital-manager animate-in !h-auto">
