...
const TownPickerModal = ({ 
    ...
    const togglePrimary = (town) => {
        const isSelected = selection.primary?.id === town.id;
        if (isSelected) {
            ...
            setSelection(prev => ({
                ...prev,
                secondary: prev.secondary.filter(t => t.id !== town.id)
            }));
        }
    };

    const toggleSecondary = (town) => {
        const isSelected = selection.secondary.some(t => t.id === town.id);
        const isPrimary = selection.primary?.id === town.id;
                    ...
                    {loading ? (
                        ...
                        <div className="towns-list-v10">
                            {filteredTowns.map(town => {
                                const isPrimary = selection.primary?.id === town.id;
                                const isSecondary = selection.secondary.some(t => t.id === town.id);
