...
const StoreManagementModule = ({ addLog }) => {
    ...
    const [ecommerceEnabled, setEcommerceEnabled] = useState(
        localStorage.getItem('GLOBAL_ECOMMERCE_ENABLED') !== 'false'
    );

    const toggleEcommerce = () => {
        const newState = !ecommerceEnabled;
        setEcommerceEnabled(newState);
        localStorage.setItem('GLOBAL_ECOMMERCE_ENABLED', newState);
        addLog(`Botiga Virtual (WooCommerce Killer) ${newState ? 'ACTIVADA' : 'DESACTIVADA'}`, newState ? 'success' : 'warn');
        ...
        <div className="stores-management-card neural-core-panel">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="automation-panel p-4 border border-gray-800 rounded-[28px] bg-black/20">
                    ...
                    <h3 className="font-bold mb-4 mt-6 text-orange-400">Botiga Virtual (Universal)</h3>
                    <div className="space-y-3">
                        <div className="flex items-center justify-between p-3 bg-gray-900/50 border border-gray-800 rounded-[20px]">
                            <div>
                                <h4 className="font-bold text-sm text-white">E-commerce Universal</h4>
                                <p className="text-[10px] text-gray-400">Activa preus, stock i compra digital al Mercat</p>
                            </div>
                            <button 
                                className={`w-12 h-6 rounded-full relative transition-colors ${ecommerceEnabled ? 'bg-green-500' : 'bg-gray-700'}`}
                                onClick={toggleEcommerce}
                            >
                                <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${ecommerceEnabled ? 'left-7' : 'left-1'}`} />
                            </button>
                        </div>
                    </div>
