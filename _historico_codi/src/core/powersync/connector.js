  ...
  async uploadData(database) {
    ...
    try {
      ...
      for (const op of transaction.crud) {
        ...
        const payload = {
          id: op.id,
          updated_at: new Date().toISOString(), // TIMESTAMP STRICT
      ...
      if (recordsToSync.length > 0) {
        ...
        const { error } = await supabaseService.supabase.rpc('process_sync_batch_v11', { 
          batch: recordsToSync 
        });
        ...
        console.log(`[SupabaseConnector] Lot CRDT de ${recordsToSync.length} elements pujat amb èxit.`);
