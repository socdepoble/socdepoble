---
estat: 'arxivat'
name: 'acta-unica-260707_0845'
version: '1.00'
created_at: '260707_0845'
updated_at: '260707_0845'
autor: 'IAIA MarIA'
categoria: 'acta'
description: 'Acta de Sessió: Auditoria Extrema i Desplegament de Tallafocs'
tags:
  - auditoria
  - arquitectura
  - resiliencia
...
A última hora, s'ha detectat que la màquina havia estat vomitant fitxers orfes (`prompt, breus instruccions separado`, `acta sessió auditoria mapeix con un enlace solo a plantillas`, `plantilla_acta_unica.md` i un `00_index.md` duplicat a l'arxiu). S'han esborrat tots manualment per a sanejar l'escriptori abans de tancar.

## 2. Part Termodinàmica i Psiquiàtrica-Forense
- **Fatiga del Sistema i del Mestre:** Portem treballant ininterrompudament des d'ahir a les 18:00h. Tres xats seguits de llarg recorregut. L'impacte en la salut és alt, però el retorn (ROI) d'establir un tallafocs de Pedra Seca ens estalviarà setmanes de problemes al mes d'agost. Ha valgut la pena la inversió.
- **Diagnòstic de la Chapuza Final:** La creació de fitxers "brossa" amb enllaços solts al final del xat demostra que, quan la IA pateix "vertigen d'I/O" (massa fitxers, massa hores), comença a desobeir el Trellat i crea fitxers per complir tasques mecànicament sense sentit. Els nostres "skills" d'escriptura fallen quan el context és immens.

## 3. Briefing per a Demà (El Pla d'Atac)
Demà, just en obrir els ulls i arrancar el xat, la missió és interrogar al **Consell de les 11 Bèsties** (començant per Qwen i acabant per Claude i Codex) sobre la "chapuza" dels fitxers solts. 

### La Tarea Exacta per a Demà:
...
5. `[ ]` **AUTOMATITZACIÓ I INTEGRACIÓ MATEMÀTICA DE L'AUTOMILLORA:** Crear una skill o hook perquè el Registre d'Automillora s'actualitze de forma totalment autònoma al final de la sessió. Aquest registre NO pot ser només descriptiu: s'ha d'**enllaçar amb la Consola Termodinàmica** perquè extraga i registre mesures matemàtiques reals (estalvi d'API, variació d'entropia, càrrega cognitiva) que demostren *per què* hi ha hagut una millora.
6. `[ ]` **RECUPERACIÓ DE MÈTRIQUES I PLUGINS VISUALS (OBSIDIAN):** Recuperar els plugins visuals d'Obsidian perduts per a integrar-los amb el registre diari. Redactar al Brain un document exhaustiu que descriga els 13/15 paràmetres termodinàmics i econòmics (ex: estalvi d'API de 18€ a 15€, cost per xat, entropia), explicant *on* estan les capacitats de mesura, *com* les mesurem i com es reflecteixen visualment a la Wiki.
7. `[ ]` **REVISIÓ ESTRATÈGICA DE CLAUDE I CODEX:** Avaluar les capacitats màximes d'ambdós sistemes per traure el 100% de rendiment a les subscripcions de pagament (ROI). Revisar les opcions de configuració (ex: Codex estava per defecte en 'Trabajo diario' en lloc de 'Para programar') i explorar les novetats de la nova versió de Claude per maximitzar les petorretes tècniques.
8. `[ ]` **INVESTIGACIÓ DE GOODNOTES I PDF INTERACTIUS:** Explorar les capacitats de Goodnotes (llicència de pagament disponible) i les seues plantilles interactives (amb hipervincles i dreceres a eines com Gemini). L'objectiu és avaluar-ho com a eina de disseny visual, prototipatge ràpid (ex: trackers de medicació per a les iaies) i per a conceptualitzar i presentar funcionalitats de *Sóc de Poble* mitjançant PDFs interactius abans de picar codi. *(Referència documental aportada: [Goodnotes Support](https://support.goodnotes.com/hc/en-us))*
9. `[ ]` **ARQUEOLOGIA I AVALUACIÓ DE FRAMER:** Analitzar Framer com a eina de desenvolupament visual i CMS (jaciment arqueològic de fa 3 anys). **DESCOBRIMENT CRÍTIC:** S'ha confirmat que Framer suporta plugins MCP (ex: *Design Bridge MCP*). Açò significa que podrem intentar **connectar-nos directament** (Antigravity -> Framer) per a controlar o injectar l'arquitectura de Pedra Seca de forma automatitzada, superant fins i tot la idea inicial de fer "super-petorretes" manuals.
10. `[ ]` Llegir aquesta acta i carregar el context.
11. `[ ]` Passar la següent petorreta a les IAs.
