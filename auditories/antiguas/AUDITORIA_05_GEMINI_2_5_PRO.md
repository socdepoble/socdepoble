# Directiva d'Auditoria Forense (Gemini 2.5 Pro (Directe a Web))

**Context:**
Sóc de Poble és una xarxa social descentralitzada (El Sistema Operatiu Rural). 
Acabem d'assolir una arquitectura molt madura (certificada com a or de producció, 10/10, per Qwen Max i DeepSeek-R1), assegurant l'aplicació contra vulnerabilitats crítiques:
1. El Llibre Major (Ledger) ara està blindat per HMAC-SHA256 (WebCrypto API) rebutjant manipulacions al client.
2. L'empaquetador de bytes a `syncService.js` usa `TextDecoder`/`Uint8Array` de manera neta d'agombols antics.
3. El Nuke de Sessió preserva hàbilment `sdp_device_id` i `sp_ledger_secret` per no matar la subsistència d'emergència a IndexedDB.
4. Redundància arquitectònica eliminada: `ThemeContext` i `DesignContext` s'han unificat netament per suprimir col·lisions CSS.

**La teua Missió Mestra (Gemini 2.5 Pro (Directe a Web)):**
Fent ús de la teua visió holística massiva (dos milions de tokens de context), analitza tota l'arquitectura global de la plataforma que t'adjuntem. Vull que avalues la cohesió entre els serveis de Supabase, l'autenticació asíncrona dual, el fail-safe del SW, la persistència de dades a l'OPFS i la sincronització CRDT offline-first. Busca errors conceptuals i punts dèbils de disseny a llarg termini. Confirma'm si aquest sistema operatiu rural està autènticament blindat per anar a producció avui amb un 10/10 definitiu.

S'adjunta a continuació tot l'Estat del Codi Màster Final (Context Generat OMEGA).


---

## CONTEXT DEL SISTEMA (Codi Font OMEGA)

```javascript
# Sóc de Poble: Arquitectura Mestra (Context per a Auditoria Codex)
...
export const AuthProvider = ({ children }) => {
    ...
    const forceNukeSimulation = useCallback(async () => {
        ...
        const ledgerSecret = localStorage.getItem('sp_ledger_secret');

        localStorage.clear();
        sessionStorage.clear();

        if (deviceId) localStorage.setItem('sdp_device_id', deviceId);
        if (ledgerSecret) localStorage.setItem('sp_ledger_secret', ledgerSecret);
...
export const ThemeProvider = ({ children }) => {
    const { theme, setTheme, toggleTheme } = useDesign();
...
export const paymentService = {
  ...
  /**
   * [FIX C3] Validació criptogràfica HMAC-SHA256 via WebCrypto API
   * Més ràpid que l'API de xarxa i blindat contra injeccions de DevTools de 32bits.
   */
  async _signEntry(entry) {
      let secretHex = localStorage.getItem('sp_ledger_secret');
      if (!secretHex) {
          const rawKey = crypto.getRandomValues(new Uint8Array(32));
          secretHex = Array.from(rawKey).map(b => b.toString(16).padStart(2,'0')).join('');
          localStorage.setItem('sp_ledger_secret', secretHex);
      }
      const keyBytes = new Uint8Array(secretHex.match(/.{2}/g).map(h => parseInt(h, 16)));
      const key = await crypto.subtle.importKey('raw', keyBytes, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
      const data = new TextEncoder().encode(`${entry.amount}|${entry.receiver_id}|${entry.type}`);
...
export const syncService = {
    ...
    unpackFromTransport: (packageData) => {
        ...
        try {
            const bytes = Uint8Array.from(atob(packageData.payload), c => c.codePointAt(0));
