...
- **Despliegue**: Dónde vive la aplicación en internet (SiteGround, Entorn Soberà).

```mermaid
graph TD
    User[Usuario] -->|Navegador| SiteGround["SiteGround (Hosting Soberà)"]
    SiteGround -->|Sirve Archivos| ReactApp["App React (Frontend)"]
...
- **Hosting**: Es donde "vive" la aplicación para que sea accesible desde internet (`socdepoble.org`).
- **Integración Continua**: Se conecta con GitHub. Cada vez que bategamos el Master Sync, SiteGround sincroniza la doctrina pública.
...
_Es posible que hayas visto referencias a esto en otros contextos técnicos, pero para "Sóc de Poble" en su estado actual, no es una pieza que estemos programando directamente._
