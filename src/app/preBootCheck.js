...
async function fetchRemoteVersion() {
  try {
    const r = await fetch(`${VERSION_ENDPOINT}?t=${Date.now()}`, {
      cache: 'no-store',
...
export async function preBootCheck({
  onStatus = () => {}
} = {}) {
  onStatus('start');
    ...
    while (tries < MAX_MIGRATION_RETRIES) {
      try {
        ...
        if (!onlineNow) {
          ...
          return {
            mode: 'readonly',
            reason: 'offline-during-migration'
          };
        }
  ...
  if (cp) {
    try {
      await runMigrations({
        ownerId: OWNER_ID,
        onProgress: p => onStatus('migration-resume', p)
      });
      ...
      onStatus('migration-resume-failed', {
        err: String(err)
      });
      scheduleBackgroundRetry();
      return {
