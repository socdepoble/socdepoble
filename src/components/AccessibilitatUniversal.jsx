...
const AccessibilitatUniversal = ({ embedded = false }) => {
  const { isAccessibilitatOpen: isOpen, setIsAccessibilitatOpen: setIsOpen } = useUI();
      ...
      <div className={`flex-1 overflow-y-auto custom-scrollbar p-12 max-w-5xl mx-auto w-full ${embedded ? 'py-8 px-8' : ''}`}>
        ...
        <section className="mb-12">
          ...
          <div className="mt-8 flex flex-col md:flex-row gap-4">
            <button onClick={handleSpeak} className="flex-1 h-20 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl flex items-center justify-center gap-3 text-xl font-bold shadow-lg transition-all">
              <Mic className="w-6 h-6" />
              <span>🔊 Llegir Pàgina</span>
            </button>
            {isSpeaking && (
              <button onClick={handleStop} className="h-20 px-10 bg-red-600 hover:bg-red-500 text-white rounded-2xl flex items-center justify-center gap-3 text-xl font-bold shadow-lg animate-in zoom-in transition-all">
                <Square className="w-6 h-6 fill-current" />
                <span>Aturar Veu</span>
              </button>
            )}
          </div>
        </section>
