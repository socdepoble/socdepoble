import { X, Calendar as CalendarIcon, Check, Settings, LogOut, Radio, PenTool, Brain, Layers } from 'lucide-react';
            ...
            <div className="calendar-modal-content" onClick={e => e.stopPropagation()}>
                <header className="calendar-modal-header">
                    <div className="title-group">
                        <Settings size={22} className="text-[var(--theme-accent-primary)]" />
                ...
                <div className="calendar-modal-body overflow-y-auto max-h-[70vh]">
                    ...
                    <div className="oauth-dashboard mb-6 pb-6 border-b border-[var(--border-master)]">
                        ...
                        <div className="calendars-list">
                            {internalCalendars.map(cal => {
                                ...
                                return (
                                    <div key={cal.id} className={`calendar-item-row ${isSelected ? 'row-active' : ''}`}>
                                        <div className="cal-info">
                                            <div 
                                                className="cal-color-dot" 
                                                style={{ backgroundColor: cal.color_id || '#169CF9' }} 
                                            />
                                        ...
                                        <div className="cal-actions">
                                            <label className="cal-visibility-toggle">
                                                <input 
                                                    type="checkbox" 
                                                    checked={isSelected}
                                                    onChange={() => toggleInternalCalendar(cal.id)}
                                                />
                            ...
                            {internalCalendars.length === 0 && (
                                <div className="p-4 text-center text-[var(--text-muted)] italic text-sm">
                                ...
                                {calendars.map(cal => {
                                    ...
                                    return (
                                        <div key={cal.id} className={`calendar-item-row ${isSelected ? 'row-active' : ''}`}>
                                            ...
                                            <div className="cal-actions">
                                                <button 
                                                    className={`btn-host-toggle ${isHost ? 'host-active' : ''}`}
                                                    onClick={() => toggleHost(cal.id)}
                                                    title="Fer d'asentament principal (Host)"
                                                >
                                                    <PenTool size={16} />
                                                </button>
