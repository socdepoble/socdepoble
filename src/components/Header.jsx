...
const Header = () => {
  ...
  return (
    <header className="h-14 flex items-center justify-between px-3 lg:px-6 gap-2 shrink-0 select-none bg-black text-theme-text sticky top-0 z-[1000] w-full border-b border-white/[0.03] transition-colors duration-300">
      <div className="flex items-center gap-1 overflow-hidden shrink-0">
        ...
        <NavLink
          to="/"
          className="flex items-center active:scale-95 transition-transform pt-4"
        >
          <img
            src="/assets/master/logo_socdepoble_white_full.png"
            alt="Sóc de Poble"
            className="w-[160px] h-auto object-contain brightness-110"
            fetchPriority="high"
          />
        </NavLink>
      </div>
      ...
      <div className="flex items-center gap-1 lg:gap-3 ml-auto h-full">
        ...
        <button
          className={`w-12 h-12 flex items-center justify-center transition-all ${
            activeLevel0
              ? "text-slate-400"
              : "text-[#FF6B00] bg-[#FF6B00]/10 rounded-full"
          } ${isOnVisionPage ? "ring-2 ring-[#FF6B00]" : ""}`}
          onClick={() => {
        ...
        >
          <IAIAIcon
            size={28}
            color={activeLevel0 ? "currentColor" : "#FF6B00"}
        ...
        {user && !user.isAnonymous ? (
          ...
          <button
            className="w-12 h-12 flex items-center justify-center text-slate-400 hover:text-[#FF6B00] transition-colors"
