...
import useSpriteAnimator from '../hooks/useSpriteAnimator'; // Mcked

const MurFeed = () => {
  // Mock data for now
  const posts = {
    docs: [
      { id: '1', data: () => ({ authorName: 'Tia Pepa', content: 'Bon dia a tots! Ja tinc la coca de llanda al forn.', likes: 12, townId: 'Bocairent' }) },
      { id: '2', data: () => ({ authorName: 'Oncle Paco', content: 'Això de les xarxes socials és una pèrdua de temps, abans anàvem al bar.', likes: 4, townId: 'Altea' }) }
    ],
    empty: false
  };
  const loading = false;
  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      ...
      <div className="space-y-6">
        {posts?.docs.map(doc => {
          ...
          return (
            <UniversalCard 
              ...
              item={post}
              variant="feed"
              onLike={() => trigger('tiaEspantaVirus', 'victory')}
              onComment={() => trigger('nanoBanana', 'enthusiasm')}
            />
          );
        })}
        ...
        {posts?.empty && (
          <div className="text-center py-20 border-2 border-dashed border-orange-300 rounded-3xl bg-orange-50/50">
