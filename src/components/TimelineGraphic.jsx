...
export default function TimelineGraphic({
  ...
  return (
    <div className='timeline-graphic p-3 bg-sdp-sdp-bg-primary rounded'>
        ...
        <div className="timeline-canvas relative border border-slate-800 rounded" style={{
        width: width,
        height: 220
      }}>
          {/* time ruler */}
          <div className="absolute left-0 top-0 right-0 h-8 border-b border-slate-800 bg-slate-900 flex items-center">
            ...
            <div className="flex-1 relative">
              ...
              bottom: 0
                ...
                {Array.from({
                length: Math.ceil(durationMs / 500) + 1
              }).map((_, i) => {
          ...
          <div className="absolute left-0 top-8 right-0 bottom-0 overflow-auto">
            ...
            position: 'relative',
            height: '100%',
            width: Math.max(width, msToPx(durationMs) + 120)
          }}>
              {/* playhead */}
              <div style={{
              position: 'absolute',
              ...
              return <div key={st.id} className={`timeline-step absolute ${isSelected ? 'selected' : ''}`} style={{
                left: x,
                top: y,
                width: Math.max(24, w),
                height: 36,
                ...
                zIndex: 40
                    ...
                    <div onMouseDown={e => onBarMouseDown(e, st.id)} onClick={() => setSelectedId(st.id)} className="bar h-full rounded flex items-center justify-between px-2" style={{
                  background: isSelected ? 'linear-gradient(90deg,#F97316,#FFB86B)' : '#0b1220',
                  border: '1px solid rgba(255,255,255,0.04)',
                  color: '#fff'
                }}>
                      <div className="text-xs truncate" style={{
                    maxWidth: Math.max(40, w - 60)
                  }}>{st.label || st.selector}</div>
                      <div className="flex items-center gap-2">
                        <div className="text-xs text-slate-300">{Math.round(st.delay || 0)}ms</div>
                        <div className="drag-handle" onMouseDown={e => onResizeMouseDown(e, st.id)} style={{
                      width: 10,
                      height: 20,
          ...
          <div className="col-span-2 p-3 bg-slate-900 rounded">
            ...
            return <div className="space-y-2">
                    <div className="flex gap-2">
                      <input value={st.label || ''} onChange={e => updateStep(st.id, {
                  label: e.target.value
                })} className="flex-1 p-2 rounded bg-slate-800 text-white" placeholder="Etiqueta" />
                      <button onClick={() => removeStep(st.id)} className="px-3 py-2 bg-red-700 rounded text-white">Eliminar</button>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      <input value={st.rootQuery} onChange={e => updateStep(st.id, {
                  rootQuery: e.target.value
                })} className="p-2 rounded bg-slate-800 text-white" />
                      <input value={st.selector} onChange={e => updateStep(st.id, {
                  selector: e.target.value
                })} className="p-2 rounded bg-slate-800 text-white" />
                      <input value={st.className} onChange={e => updateStep(st.id, {
                  className: e.target.value
                })} className="p-2 rounded bg-slate-800 text-white" />
                    </div>
                    <div className="flex items-center gap-2">
                      <label className="text-xs text-slate-400">Delay</label>
                      <input type="range" min="0" max={durationMs} value={st.delay || 0} onChange={e => updateStep(st.id, {
                  delay: Number(e.target.value)
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input type="range" min="80" max={durationMs} value={st.duration || 800} onChange={e => updateStep(st.id, {
