...
export default function TimelineMobile({
  ...
  const [steps, setSteps] = useState(initialSteps.map((s,i)=>({ ...s, id: s.id || `m${i}`})));
  ...
  function startDragDelay(e, id) {
    ...
    function onMove(ev) {
      ...
      updateStep(id, { delay: snap(newDelay) });
    }
    function onEnd() {
  ...
  function startResizeDuration(e, id) {
    ...
    function onMove(ev) {
      ...
      updateStep(id, { duration: snap(newDuration) });
    }
    function onEnd() {
      window.removeEventListener("touchmove", onMove);
      window.removeEventListener("mousemove", onMove);
  ...
  function runTimeline() {
    ...
    const blocks = steps.map(st => {
      ...
      const block = { root, steps: [stepObj], offset: cumulative };
  ...
  return (
    <div ref={containerRef} className="timeline-mobile p-3 bg-slate-900 rounded-2xl shadow-xl border border-slate-800">
      ...
      <div className="space-y-3">
        {steps.map((st, i) => (
          <div key={st.id} className="bg-slate-800 p-3 rounded-xl flex items-center gap-3 border border-slate-700">
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 relative h-12 bg-slate-900 rounded-lg overflow-hidden border border-slate-700" onTouchStart={(e)=>startDragDelay(e, st.id)} onMouseDown={(e)=>startDragDelay(e, st.id)}>
                <div style={{
                  ...
                  <div onTouchStart={(e)=>startResizeDuration(e, st.id)} onMouseDown={(e)=>startResizeDuration(e, st.id)} style={{ width: 18, height: 28, background: 'rgba(0,0,0,0.15)', borderRadius: 4, cursor: 'ew-resize' }} />
                </div>
              </div>

              <div className="mt-3 flex items-center gap-2">
                <div className="text-xs text-slate-400 font-medium">Delay</div>
                <input type="range" min="0" max="5000" step={snapMs} value={st.delay || 0} onChange={e => updateStep(st.id, { delay: Number(e.target.value) })} className="flex-1 accent-orange-500" />
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <input type="range" min="50" max="5000" step={snapMs} value={st.duration || 800} onChange={e => updateStep(st.id, { duration: Number(e.target.value) })} className="flex-1 accent-orange-500" />
