...
const ChatDetail = ({ embeddedId }) => {
  ...
  }, [location.state, id, handleSendMessage, safeSetTimeout]);
...
// ==========================================
// SUBCOMPONENTS (Shells para reconstruir)
// ==========================================

const ChatHeader = React.memo(({ otherInfo, isHeaderSearchOpen, setIsHeaderSearchOpen, searchQuery, setSearchQuery, isSettingsMenuOpen, setIsSettingsMenuOpen, isEmbedded }) => {
  return (
    <div className="sticky top-0 z-[190] w-full shrink-0 shadow-sm">
      <UniversalCardHeader
          item={otherInfo}
          cardVariant="chat"
          displayTown={otherInfo?.town_name || otherInfo?.town || 'La Torre de les Maçanes'}
          displayAuthor={otherInfo?.name || 'Sóc de Poble'}
          avatarSrc={otherInfo?.avatar_url}
          avatarRole={otherInfo?.role}
          isOfficial={true}
          isPageHeader={true}
          displayTime={new Date().toLocaleTimeString('ca-ES', { hour: '2-digit', minute: '2-digit' })}
          className="!h-[56px] !min-h-[56px] !max-h-[56px]"
      />
    </div>
  );
});
...
const ChatInputArea = React.memo(({ handleSendMessage, isSending }) => {
...
const ChatMoveSelectorModal = React.memo(({ msg, onClose, onSelect }) => {
  return null;
});
