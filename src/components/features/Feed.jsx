...
const Feed = ({ townId = null, townName = null, customPosts = null, contentMode = 'batec', hideHeader = false, externalViewMode = null }) => {
    ...
    const rowVirtualizer = useVirtualizer({
        ...
        overscan: 2,
        onChange: (instance) => {
            const lastIndex = (instance.getVirtualItems()[instance.getVirtualItems().length - 1])?.index ?? 0;
    ...
    useEffect(() => {
        ...
        // El text flex-auto canvia d'alçada quan es carrega la lletra (Noto Sans). 
        // Si no forcem react-virtual a remesurar, la fila es queda amb l'alçada original (abans de carregar la lletra)
        // i les targetes se solapen 100% garantit.
        ...
        // [ANTI-OVERLAP] Forcem la mesura inicial quan carreguen les fonts i layout inicial.
        const t1 = setTimeout(() => rowVirtualizer.measure(), 100);
        const t2 = setTimeout(() => rowVirtualizer.measure(), 500);
    ...
    const handleHeaderClick = useCallback((post) => {
        ...
        const type = (post.type === 'ajuntament' || post.author_type === 'ajuntament') ? 'ajuntament' : (post.author_entity_id ? 'empresa' : 'gent');
        ...
        if (post.author_is_ai || post.is_iaia_inspired || targetId === IAIA_ID) {
            navigate('/iaia');
            return;
        }
    ...
    const FeedPostCard = React.memo(({ post, effectiveViewMode, handleHeaderClick, gloveMode }) => {
        ...
        const headerTitle = post.metadata?.is_verified
            ...
            : (post.author?.name || post.author || post.author_name || 'Nano Banana');
        ...
        return (
            <div className={`card-rizoma-wrapper flex-auto ${isDissolving ? 'dissolve' : ''} w-full flex flex-col`}>
                <UniversalCard
                    item={post}
