...
import MarketSkeleton from '../ui/skeletons/MarketSkeleton';
...
const MarketCard = React.memo(({ item, viewMode, handleHeaderClick, handleRecipeClick }) => {
    const imageSources = item.image_url || item.images || item.image || '/assets/mock-data/illustrations/generic-market.png';
    const headerTitle = item.seller_name || item.seller || 'Sóc de Poble';
    
    return (
        <div className="card-rizoma-wrapper w-full h-full">
            <UniversalCard
                item={item}
...
const Market = ({ searchTerm = '' }) => {
    ...
    const filteredItems = useMemo(() => {
        ...
        if (visionMode === 'humana' && !isSuperAdmin) {
            baseItems = baseItems.filter(item => {
                ...
                if (BLACKLIST_NAMES.some(name => nameToCheck.toUpperCase().includes(name.toUpperCase()))) return false;
    ...
    const handleHeaderClick = (item) => {
        ...
        const type = (item.type === 'ajuntament' || item.author_type === 'ajuntament') ? 'ajuntament' : (item.author_entity_id ? 'empresa' : 'gent');
        ...
        if (item.author_is_ai || item.is_iaia_inspired || targetId === IAIA_ID) {
            navigate('/iaia');
            return;
        }
    ...
    return (
        <RootPageContainer className="bg-white w-full" ref={containerRef}>
            ...
            <div 
                ref={scrollRef} 
                className="flex-1 overflow-y-scroll custom-scrollbar pb-24 w-full relative"
            >
                            ...
                            return (
                                <div
                                    ...
                                    style={{
                                        ...
                                        paddingBottom: '24px' // Marge de seguretat
                                    }}
                                >
                                    <UniversalGridRow
                                        viewMode={viewMode}
