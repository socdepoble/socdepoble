...
export default function LocalFirstGate({
  children
}) {
    ...
    const initDb = async () => {
      ...
      if (lastSchema && lastSchema !== currentSchemaVersion) {
        ...
        try {
          ...
          await root.removeEntry("socdepoble.db", {
            recursive: true
          }).catch(() => null);
        } catch (err) {}
      ...
      if (!dbRef.current) {
        ...
        } = await import('../../utils/storageProbe');
      ...
      try {
        const initTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error("TIMEOUT_OPFS")), OPFS_TIMEOUT_MS));
        ...
        const connectTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error("TIMEOUT_CONNECT")), OPFS_TIMEOUT_MS + 1500));
  ...
  if (status === STATUS.ERROR) {
    return <div className="bg-[#111827] text-white min-h-screen flex items-center justify-center flex-col p-6 text-center">
  ...
  if (status === STATUS.IDLE) {
    return <div className="min-h-screen flex items-center justify-center flex-col relative overflow-hidden" style={{
      background: "#0b0b0b"
    }}>
        <div className="absolute inset-0 bg-gradient-to-br from-[#00f2ff]/5 to-transparent pointer-events-none" />
        <BrandLogo className="h-20 w-auto mb-8 opacity-80 animate-pulse text-white" style={{
        filter: "drop-shadow(0 0 10px rgba(0,242,255,0.3))"
      }} />
        <div className="flex justify-center gap-2 mb-6">
          {[0, 150, 300].map(delay => <div key={delay} className="w-1.5 h-1.5 rounded-full bg-[#00f2ff] animate-pulse opacity-80" style={{
          animationDelay: `${delay}ms`
        }} />)}
        </div>
        <p className="text-[#00f2ff] text-xs font-black uppercase tracking-[0.2em] opacity-70">Connectant...</p>
      </div>;
  }
  return <LocalFirstStatusContext.Provider value={contextValue}>
