...
const CreateEventModal = ({
  ...
  const [form, setForm] = useState({
    ...
    timeStart: '',
    type: 'festa',
    // festa | recollida | assemblea | mercat
    location: '',
    emoji: '🌾'
  });
  ...
  return <motion.div initial={{
    ...
    opacity: 0
      ...
      y: 0
            ...
            title: e.target.value
          })} placeholder="Festa major de taronges" className="w-full px-5 py-5 text-2xl rounded-2xl border-2 border-transparent focus:border-[#F97316] outline-none bg-black/10 dark:bg-black/30" required aria-required="true" />
              ...
              ...form,
              date: e.target.value
            })} className="w-full px-5 py-5 rounded-2xl bg-black/10 dark:bg-black/30 outline-none focus:ring-2 ring-[#F97316]" required />
                        </div>
                        <div>
                            <label className="block text-sm font-bold mb-2">Hora (opcional)</label>
                            <input type="time" value={form.timeStart} onChange={e => setForm({
              ...form,
              timeStart: e.target.value
            })} className="w-full px-5 py-5 rounded-2xl bg-black/10 dark:bg-black/30 outline-none focus:ring-2 ring-[#F97316]" />
                        </div>
                    </div>
            ...
            ...form,
            type: e.target.value
          })} className="flex-1 px-5 py-5 rounded-2xl bg-black/10 dark:bg-black/30 outline-none focus:ring-2 ring-[#F97316]">
                            <option value="festa">🌾 Festa</option>
                            <option value="recollida">🍊 Recollida</option>
                            <option value="assemblea">🗣️ Assemblea</option>
                            <option value="mercat">🛒 Mercat</option>
                        </select>
                        <input type="text" maxLength={2} value={form.emoji} onChange={e => setForm({
            ...form,
        ...
        })} placeholder="Explica’ns una mica més..." className="w-full h-32 px-5 py-5 rounded-3xl resize-none bg-black/10 dark:bg-black/30 outline-none focus:ring-2 ring-[#F97316]" />
          ...
          })} placeholder="Plaça del Poble" className="w-full px-5 py-5 rounded-2xl bg-black/10 dark:bg-black/30 outline-none focus:ring-2 ring-[#F97316]" />
                    </div>

                    <TactileButton type="submit" disabled={isSubmitting} className="w-full py-7 text-2xl font-black bg-[#F97316] text-white rounded-3xl flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50">
