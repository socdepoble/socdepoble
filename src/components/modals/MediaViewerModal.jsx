 ...
 */
const MediaViewerModal = ({ isOpen, onClose, src, title, type = 'image', images = [], imageDescriptions = [], onNavigate }) => {
    ...
    const currentDescription = hasCarousel && imageDescriptions && imageDescriptions[currentIndex] 
        ...
        <div className="media-viewer-overlay" onClick={onClose}>
            <div className="media-viewer-toolbar" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="toolbar-right">
                    {type === 'image' && (
                        <button className={`toolbar-btn ${showInfo ? 'active-info' : ''}`} onClick={toggleInfo} title="Info i EXIF">
            ...
            <div className="media-viewer-content" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="viewer-main-media-container relative h-full w-full flex items-center justify-center">
                    ...
                    {type === 'image' && imageError && (
                        <div className="viewer-main-media flex items-center justify-center bg-[var(--theme-surface-sunken)] flex-col text-[var(--theme-text-muted)] p-8 text-center rounded-2xl w-full h-full min-h-[300px] max-w-lg mx-auto">
                    ...
                    {showInfo && type === 'image' && (
                        <div className="exif-info-panel absolute right-0 top-0 bottom-0 bg-black/80 backdrop-blur-md p-6 overflow-y-auto w-full md:w-80 text-white border-l border-white/10 animate-fade-in z-20">
                            <h3 className="text-xl font-bold mb-4 flex items-center gap-2 border-b border-white/20 pb-2">
                                <Info size={20} /> Metadades
                            </h3>
                            
                            {currentDescription && (
                                <div className="mb-6 bg-white/10 p-3 rounded-lg">
                            ...
                            {isExtractingExif ? (
                                <div className="text-center py-8 text-white/60 animate-pulse">Llegint dades EXIF...</div>
                            ) : exifData ? (
                                <div className="space-y-4">
                                    {exifData.Make || exifData.Model ? (
                                        <div className="exif-group bg-white/5 p-3 rounded-lg">
            ...
            <div className="media-viewer-footer" onClick={(e) => e.stopPropagation()}>
                {type === 'image' ? (
                    currentDescription ? currentDescription : (hasCarousel ? 'Usa les fletxes de navegació. Clic fora per tancar.' : 'Mantín per desa o copia. Clic fora per tancar.')
                ) : 'Navega lliurement pel mapa local.'}
            </div>
        </div>
    );
