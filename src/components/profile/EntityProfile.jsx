...
import { MapPin, BadgeCheck, Settings, Users, Grid, Store, ArrowLeft, MessageCircle, Share2, Plus, Download, Contact, Info, FileText, Square, LayoutGrid, List } from 'lucide-react';
...
export default function EntityProfile({
  ...
  const {
    viewMode,
    setViewMode
  } = useViewMode('entity_profile_view_mode', 'grid');

  // Reset imgError if the banner URL changes (e.g. hot reload or profile switch)
  React.useEffect(() => {
    setImgError(false);
  }, [entity?.profile?.bannerUrl]);
  ...
  const userPosts = useMemo(() => {
    ...
    return allPosts.filter(p => {
      // 1. Global pinned posts should NOT be shown in individual profiles.
      // (Removed the is_pinned check that was here)

      // 2. If this is a TOWN profile (Gent de La Torre), show EVERYTHING from this town
      if (entity.is_town) {
        const normalizeStr = s => (s || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
        const isTownMatch = entity.town_name && p.town_name && (normalizeStr(p.town_name) === normalizeStr(entity.town_name) || normalizeStr(p.town_name).includes(normalizeStr(entity.town_name)) || normalizeStr(entity.town_name).includes(normalizeStr(p.town_name)));

        // Match by town ID just in case
        const isTownIdMatch = entity.raw_town_id && (String(p.town_uuid) === String(entity.raw_town_id) || String(p.town_id) === String(entity.raw_town_id));
        return isTownMatch || isTownIdMatch;
      }

      // 3. If this is a USER/ENTITY profile (Ajuntament, Sóc de Poble, etc), ONLY show their own posts
      ...
      return isDirectMatch || isNameMatch || isAjuntamentTypeMatch;
    });
  }, [allPosts, entity]);
  ...
  return (
    <div className="atom-root bg-theme-base min-h-screen pb-20">
        ...
        <div className="sticky top-0 z-sticky w-full max-w-full overflow-hidden shadow-lg bg-[#4F46E5] text-white transition-all shrink-0">
            <div className="flex items-center justify-between min-h-[56px] px-2 sm:px-4 w-full max-w-7xl mx-auto overflow-hidden">
                ...
                <div className="flex items-center justify-end gap-1 sm:gap-2 flex-1 min-w-0 overflow-x-auto no-scrollbar scroll-smooth">
                    <button className="flex items-center justify-center gap-1.5 min-h-[44px] px-2 sm:px-3 rounded-xl hover:bg-white/20 active:scale-95 transition-colors touch-manipulation font-bold uppercase text-sm shrink-0" aria-label="Traduir Pàgina" onClick={() => openTranslationModal({
              postId: entity?.id,
              title: entity?.profile?.displayName
                    ...
                    <button className="flex items-center justify-center gap-2 min-h-[44px] px-2 sm:px-3 hover:bg-white/20 rounded-xl active:scale-95 touch-manipulation font-bold uppercase text-sm shrink-0" aria-label="Comentar" onClick={() => {
              ...
              navigate(`/chats/${safeHandle}`);
                    ...
                    <button className="flex items-center justify-center gap-2 min-h-[44px] px-2 sm:px-3 hover:bg-white/20 rounded-xl active:scale-95 touch-manipulation font-bold uppercase text-sm shrink-0" aria-label="Compartir Perfil" onClick={() => {
              if (navigator.share) navigator.share({
                title: profile.displayName || 'Sóc de Poble',
                url: window.location.href
                    ...
                    {!isOwner && <button className="btn-action-primary" onClick={() => navigate('/connectar')}>
        ...
        <div role="region" aria-label="Capçalera de Secció" className="relative w-full h-48 sm:h-64 md:h-80 bg-black/10 dark:bg-white/5">
          ...
          <div className="absolute -bottom-4 right-6 flex justify-end items-end z-[201]">
            {isOwner && <button onClick={onSettingsClick} className="btn-tactile bg-theme-panel text-theme-text border border-border-master px-4 py-2 text-sm font-bold flex items-center rounded-2xl shadow-lg hover:scale-105 active:scale-95 transition-all">
        ...
        <div className="mt-6 px-4 sm:px-6 max-w-4xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
              <div className="flex flex-row items-center gap-4">
                  <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full border-4 border-theme-panel bg-theme-panel overflow-hidden shadow-sm glass-rural shrink-0">
                    {profile.avatarUrl ? <div className={`w-full h-full flex items-center justify-center ${profile.role === 'ajuntament' || profile.avatarUrl && profile.avatarUrl.toLowerCase().includes('logo') ? 'bg-white p-1.5 sm:p-2' : ''}`}>
                        <img src={profile.avatarUrl} alt={profile.displayName || "Avatar"} className={profile.role === 'ajuntament' || profile.avatarUrl && profile.avatarUrl.toLowerCase().includes('logo') ? 'w-full h-full object-contain' : 'w-full h-full object-cover'} />
                      </div> : <div className="w-full h-full flex items-center justify-center bg-black/10 dark:bg-white/10 text-3xl font-black text-theme-text opacity-50">
          ...
          {profile.location && <p className='text-sdp-text-muted flex items-center text-sm font-semibold mb-4 mt-2'>
          ...
          {rawBio && (rawBio.includes("demostració tècnica") || rawBio.includes("pàgina de prova")) ? <div className="my-7 bg-[#FF6D23]/10 dark:bg-[#FF6D23]/15 border border-[#FF6D23]/40 dark:border-[#FF6D23]/60 rounded-xl px-4 py-3 sm:px-5 sm:py-3.5 flex flex-col sm:flex-row items-center sm:items-start gap-3 shadow-sm relative overflow-hidden max-w-4xl">
            ...
            </div> : <div className="text-base leading-relaxed text-theme-text max-w-2xl mb-6 mt-4 app-cms-content prose prose-lg dark:prose-invert">
              {bioIntro.split('\n\n').map((paragraph, idx) => <p key={`bio-${idx}`} className="whitespace-pre-line last:mb-0" dangerouslySetInnerHTML={{
          ...
          <div className="flex items-center justify-between border-b border-black/10 dark:border-white/10 mb-6 overflow-x-auto no-scrollbar scroll-smooth w-full mt-4">
            <div className="flex gap-1 sm:gap-2">
              ...
              {(type === 'town' || entity?.is_town) && <TabButton active={activeTab === 'identitat'} onClick={() => setActiveTab('identitat')} icon={<ImageIcon size={18} />} label="Identitat" />}
              <TabButton active={activeTab === 'vcard'} onClick={() => setActiveTab('vcard')} icon={<Contact size={18} />} label="vCard" />
              {type === 'empresa' && <TabButton active={activeTab === 'store'} onClick={() => setActiveTab('store')} icon={<Store size={18} />} label="Aparador" />}
            </div>
            
            {/* VIEW MODE SWITCH */}
            <div className="flex gap-1 pr-2 pb-2 shrink-0">
                <button onClick={() => setViewMode('single')} className={`p-2 rounded-xl transition-all flex items-center justify-center ${viewMode === 'single' ? 'bg-[#F97316] text-white shadow-sm' : 'text-black/50 dark:text-white/50 hover:bg-black/5 dark:hover:bg-white/5'}`} title="Vista Completa (1 Columna)">
                
                    <Square size={18} strokeWidth={2.5} />
                </button>
                <button onClick={() => setViewMode('grid')} className={`p-2 rounded-xl transition-all flex items-center justify-center ${viewMode === 'grid' ? 'bg-[#F97316] text-white shadow-sm' : 'text-black/50 dark:text-white/50 hover:bg-black/5 dark:hover:bg-white/5'}`} title="Vista Graella (Columnes)">
                
                    <LayoutGrid size={18} strokeWidth={2.5} />
                </button>
                <button onClick={() => setViewMode('list')} className={`p-2 rounded-xl transition-all flex items-center justify-center ${viewMode === 'list' ? 'bg-[#F97316] text-white shadow-sm' : 'text-black/50 dark:text-white/50 hover:bg-black/5 dark:hover:bg-white/5'}`} title="Vista Llista (Compacta)">
          ...
          <div className="stable-scroll min-h-[40vh] pb-10">
             {activeTab === 'feed' && <div className="w-full">
                 {displayPosts.length > 0 ? <div className="min-h-[500px]">
                     <Feed hideHeader={true} customPosts={displayPosts} externalViewMode={viewMode} isProfileOwner={isOwner} />
                   </div> : <div className='text-sdp-text-muted text-sm text-center py-10 glass-rural rounded-2xl mx-2 mt-4'>
             ...
             {activeTab === 'document' && bioDocument && <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                 <ContentWithShortcodes content={bioDocument} hideTabs={true} />
               </div>}
             {activeTab === 'info' && <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                 <ConnectionsTab />
                 <TraitsViewer traits={traits} type={type} />
               </div>}
             {activeTab === 'identitat' && (type === 'town' || entity?.is_town) && <TownProposalsTab entity={entity} />}
             {activeTab === 'vcard' && <VCardTab entity={entity} profile={profile} />}
             {activeTab === 'store' && type === 'empresa' && <div className='text-sdp-text-muted text-sm text-center py-10 glass-rural rounded-2xl mx-2'>
...
const TabButton = ({
  ...
  icon,
  label
}) => <button onClick={onClick} aria-label={label} title={label} className={`flex items-center justify-center gap-2 px-3 sm:px-4 py-3 font-bold text-sm tracking-wide transition-all rounded-t-2xl whitespace-nowrap flex-1 lg:flex-none
    ...
    <div className="glass-rural p-6 rounded-3xl mx-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-1">
            <span className='text-xs uppercase font-bold text-sdp-text-muted tracking-wider'>Nom i Organització</span>
            <span className="text-lg font-bold text-theme-text">{profile.displayName || 'Entitat Desconeguda'}</span>
          </div>

          {entity?.contact_email && <div className="flex flex-col gap-1">
              <span className='text-xs uppercase font-bold text-sdp-text-muted tracking-wider'>Correu Electrònic</span>
              <span className="text-base font-medium text-theme-text">{entity.contact_email}</span>
            </div>}

          {entity?.contact_phone && <div className="flex flex-col gap-1">
              <span className='text-xs uppercase font-bold text-sdp-text-muted tracking-wider'>Telèfon Directe</span>
              <span className="text-base font-medium text-theme-text">{entity.contact_country_code || '+34'} {entity.contact_phone}</span>
            </div>}

          {profile?.location && <div className="flex flex-col gap-1">
              <span className='text-xs uppercase font-bold text-sdp-text-muted tracking-wider'>Adreça o Ubicació</span>
              <span className="text-base font-medium text-theme-text">{profile.location}</span>
            </div>}

          {profile?.bio && <div className="flex flex-col gap-1">
              <span className='text-xs uppercase font-bold text-sdp-text-muted tracking-wider'>Nota Administrativa</span>
              <span className='text-sm text-sdp-text-muted leading-relaxed'>{profile.bio}</span>
            </div>}
        </div>

        <div className="mt-10 pt-6 border-t border-border-master flex justify-end">
          <button onClick={handleDownload} className="flex items-center justify-center gap-2 px-6 py-3 min-h-[48px] rounded-xl font-black text-base transition-all bg-theme-panel text-theme-text shadow-lg hover:scale-105 active:scale-95 border border-border-master hover:border-theme-accent-primary hover:text-theme-accent-primary">
    ...
    <div className="glass-rural p-6 rounded-3xl mx-2 shadow-sm border border-black/5 dark:border-white/5 mb-6">
        ...
        <div className="bg-[#FF6D23]/10 border border-[#FF6D23]/20 dark:border-[#FF6D23]/30 rounded-2xl p-4 flex flex-col sm:flex-row items-center gap-4 relative overflow-hidden">
          ...
          <div className="text-center sm:text-left flex-1 relative z-10">
            ...
            <p className='text-[11px] sm:text-xs text-sdp-text-muted font-black uppercase tracking-wider'>Connectat · Membre de la Malla</p>
          </div>
          <div className="shrink-0 mt-2 sm:mt-0 relative z-10">
            <span className="bg-[#FF6D23] text-white text-xs uppercase font-black px-3 py-1.5 rounded-full tracking-wider shadow-md">
              Autoritzat
            </span>
          </div>
        </div>
...
const TraitsViewer = ({
  ...
  if (!traits || Object.keys(traits).length === 0) {
    return (
      <div className='glass-rural p-6 rounded-3xl mx-2 text-center text-sm font-bold text-sdp-text-muted'>
  ...
  return (
    <div className="glass-rural p-6 rounded-3xl mx-2 shadow-sm border border-black/5 dark:border-white/5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {Object.entries(traits).map(([key, value]) => <div key={key} className="bg-black/5 dark:bg-white/5 p-4 rounded-2xl border border-black/5 dark:border-white/5 overflow-hidden">
              <span className='text-[11px] uppercase tracking-wider text-sdp-text-muted font-black block mb-1'>
                {key.replace(/([A-Z])/g, ' $1').trim()}
              </span>
              <span className="text-sm font-bold text-theme-text break-words">
                {Array.isArray(value) ? <div className="flex flex-wrap gap-2 mt-2">
                    {value.length > 0 ? value.map((v, i) => <span key={i} className="bg-black/10 dark:bg-white/10 px-2 py-1 rounded-md text-xs">
                        {typeof v === 'object' && v !== null ? JSON.stringify(v) : String(v)}
                      </span>) : <span className="opacity-50 italic">Cap valor definit</span>}
                  </div> : typeof value === 'object' && value !== null ? <pre className="text-xs whitespace-pre-wrap font-mono mt-2 bg-black/5 dark:bg-white/5 p-2 rounded-lg overflow-x-auto">
                    {JSON.stringify(value, null, 2)}
                  </pre> : value !== undefined && value !== null && value !== '' ? String(value) : <span className="opacity-50 italic">Pendent de definició</span>}
              </span>
            </div>)}
        </div>
      </div>
