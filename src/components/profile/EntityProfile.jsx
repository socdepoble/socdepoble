...
import { MapPin, BadgeCheck, Settings, Users, Grid, Store, ArrowLeft, MessageCircle, Share2, Plus, Download, Contact, Info, FileText, Square, LayoutGrid, List, ImageIcon } from 'lucide-react';
        ...
        <div className="mt-6 px-4 sm:px-6 max-w-4xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
              <div className="flex flex-row items-center gap-4">
                  <div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-full border-4 border-white bg-white shadow-sm shrink-0">
                    {profile.avatarUrl ? (
                      <div className={`w-full h-full flex items-center justify-center rounded-full overflow-hidden ${profile.role === 'ajuntament' || profile.avatarUrl.toLowerCase().includes('logo') ? 'bg-white p-2' : ''}`}>
                        <img src={profile.avatarUrl} alt={profile.displayName || "Avatar"} className={`w-full h-full ${profile.role === 'ajuntament' || profile.avatarUrl.toLowerCase().includes('logo') ? 'object-contain' : 'object-cover'}`} />
                      </div>
                    ) : (
          ...
          <div className="min-h-[40vh] pb-10">
             ...
             {activeTab === 'store' && type === 'empresa' && (
                <div className='text-gray-500 text-sm text-center py-10 bg-gray-50 rounded-2xl border border-gray-200 mx-2'>
