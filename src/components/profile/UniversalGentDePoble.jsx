...
import { MapPin, BadgeCheck, Settings, Users, Grid, ArrowLeft, MessageCircle, Share2, Plus, Download, Contact, Info, FileText, Square, LayoutGrid, List, ImageIcon } from 'lucide-react';
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
        ...
        <div className="relative w-full h-48 sm:h-64 md:h-80 bg-gray-100">
          ...
          {displayCover ? <img src={displayCover} alt="Banner" className="w-full h-full object-cover" loading="lazy" onError={() => setImgError(true)} /> : <div className="w-full h-full bg-gradient-to-r from-gray-200 to-gray-50" />}
          <div className="absolute bottom-2 right-2 bg-black/40 backdrop-blur-md px-2 py-1 rounded text-xs text-white/90 font-medium z-10">
             {coverAuthor}
          </div>
        </div>
        ...
        <div className="mt-6 px-4 sm:px-6 max-w-4xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
              <div className="flex flex-row items-center gap-4">
                  <div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-full border-4 border-white bg-white shadow-sm shrink-0 group">
                    {displayAvatar ? (
                      <div className={`w-full h-full flex items-center justify-center rounded-full overflow-hidden ${displayAvatar.toLowerCase().includes('logo') ? 'bg-white p-2' : ''}`}>
          ...
          <div className="min-h-[40vh] pb-10">
             ...
             {activeTab === 'identitat' && (
                <div className="space-y-4">
                  ...
                  <p className='text-center text-xs text-gray-500 italic m-0'>
