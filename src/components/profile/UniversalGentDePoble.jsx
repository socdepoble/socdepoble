...
export default function UniversalGentDePoble({
  entity,
  isOwner,
  onSettingsClick
}) {
  ...
  const {
    winners
  } = useTownProposals(entity?.raw_town_id || entity?.id);
  ...
  return (
    <div className="atom-root bg-theme-base min-h-screen pb-20">
        ...
        <div role="region" aria-label="Capçalera de Secció" className="relative w-full h-48 sm:h-64 md:h-80 bg-black/10 dark:bg-white/5">
          {isOwner && <div className="absolute top-4 right-4 z-[201]">
                  <button onClick={onSettingsClick} className="btn-tactile bg-theme-panel text-theme-text border border-border-master px-4 py-2 text-sm font-bold flex items-center rounded-2xl shadow-lg hover:scale-105 active:scale-95 transition-all">
              
                    <Settings size={18} className="mr-2" />
                    Configurar Identitat
                  </button>
              </div>}
          {displayCover ? <img src={displayCover} alt="Banner" className="w-full h-full object-cover" loading="lazy" onError={() => setImgError(true)} /> : <div className="w-full h-full bg-gradient-to-r from-theme-panel to-theme-base opacity-50" />}
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-theme-base opacity-90" />
          <div className="absolute bottom-2 right-2 bg-black/40 backdrop-blur-md px-2 py-1 rounded text-xs text-white/80 font-medium z-10">
             {coverAuthor}
          </div>
        </div>
        ...
        <div className="mt-6 px-4 sm:px-6 max-w-4xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
              <div className="flex flex-row items-center gap-4">
                  <div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-full border-4 border-theme-panel bg-theme-panel shadow-sm glass-rural shrink-0 group" data-active="false" onPointerEnter={(e) => e.currentTarget.dataset.active = "true"} onPointerLeave={(e) => e.currentTarget.dataset.active = "false"} onPointerCancel={(e) => e.currentTarget.dataset.active = "false"}>
                    {displayAvatar ? <div className={`w-full h-full flex items-center justify-center rounded-full overflow-hidden ${displayAvatar.toLowerCase().includes('logo') ? 'bg-white p-1.5 sm:p-2' : ''}`}>
                      ...
                      </div> : <div className="w-full h-full flex items-center justify-center bg-black/10 dark:bg-white/10 text-3xl font-black text-theme-text opacity-50 rounded-full">
                        {(profile.displayName || "NN").substring(0, 2).toUpperCase()}
                      </div>}
                    <div className='absolute -bottom-2 -right-2 bg-theme-base shadow-sm border border-black/5 dark:border-white/5 rounded px-1.5 py-0.5 text-[8px] font-bold text-sdp-text-muted whitespace-nowrap opacity-0 group-data-[active=true]:opacity-100 transition-opacity'>
                      {avatarAuthor}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col justify-center">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          {lemaAuthor && <span className='bg-black/5 dark:bg-white/5 text-xs px-1.5 py-0.5 rounded text-sdp-text-muted'>{lemaAuthor}</span>}
                      </div>
                  </div>
              </div>
          ...
          {rawBio && (rawBio.includes("demostració tècnica") || rawBio.includes("pàgina de prova")) ? <div className="my-7 bg-[#FF6D23]/10 dark:bg-[#FF6D23]/15 border border-[#FF6D23]/40 dark:border-[#FF6D23]/60 rounded-xl px-4 py-3 sm:px-5 sm:py-3.5 flex flex-col sm:flex-row items-center sm:items-start gap-3 shadow-sm relative overflow-hidden max-w-4xl">
               ...
               <div className="flex-1 z-10">
                   <div className="flex items-center gap-2">
                      ...
                      {textAuthor && <span className="bg-[#FF6D23]/20 text-[#FF6D23] text-xs px-1.5 py-0.5 rounded font-bold">{textAuthor}</span>}
                   </div>
          ...
          <div className="stable-scroll min-h-[40vh] pb-10">
             ...
             {activeTab === 'identitat' && <div className="flex flex-col gap-2">
                 <TownProposalsTab entity={entity} />
                 <div className='mx-4 mb-6 text-center text-xs text-sdp-text-muted italic'>
