...
const ConnectButton = ({ className = "" }) => {
    ...
    if (user && !user.isAnonymous) {
        return (
            <div
                className={`shrink-0 w-[40px] h-[40px] flex items-center justify-center cursor-pointer hover:scale-105 transition-transform active:scale-95 ${className}`}
    ...
    return (
        <button
            className={`shrink-0 w-[40px] h-[40px] flex items-center justify-center hover:bg-white/20 active:scale-95 transition-colors rounded-xl text-white touch-manipulation ${className}`}
            onClick={() => navigate("/registre")}
            title={t("nav.register") || "Connectar"}
