...
const ContextualMenu = () => {
  ...
  const menuConfigs = {
    '/chats': [{
      ...
      id: 'grups',
      label: 'GRUPS',
      path: '/chats?tab=grups'
    }, {
      id: 'empreses',
      label: 'EMPRESES',
      path: '/chats?tab=empreses'
    }, {
      id: 'institucions',
      label: 'INSTITUCIONS',
      path: '/chats?tab=institucions'
    }],
    '/pobles': [{
      id: 'pobles',
      label: 'POBLES',
    ...
    '/calendar': [{
      ...
      id: 'personal',
      label: 'PERSONAL',
      path: '/calendar?role=personal'
    }, {
      id: 'empresa',
      label: 'EMPRESA',
      path: '/calendar?role=empresa'
    }, {
      id: 'treball',
      label: 'TREBALL',
      path: '/calendar?role=treball'
    }, {
      id: 'estudis',
      label: 'ESTUDIS',
      path: '/calendar?role=estudis'
    }]
  };
  ...
  const profileMenu = [{
    ...
    id: 'personal',
    label: 'PERSONAL',
    path: `${location.pathname}?role=personal`
  }, {
    id: 'autonom',
    label: 'AUTÒNOM/EST',
    path: `${location.pathname}?role=autonom`
  }, {
    id: 'empresa',
    label: 'EMPRESA',
    path: `${location.pathname}?role=empresa`
  }, {
    id: 'grup',
    label: 'GRUP',
    path: `${location.pathname}?role=grup`
  }, {
    id: 'entitat',
    label: 'ENTITAT',
    path: `${location.pathname}?role=entitat`
  }];
  ...
  return (
    <div className="h-[48px] min-h-[48px] max-h-[48px] w-full bg-[#1a1a1a] shadow-[inset_0_4px_10px_rgba(0,0,0,0.6)] flex items-center sticky top-0 z-sticky select-none overflow-hidden shrink-0">
            ...
            navigate(item.path);
                                  ...
                                  ${isProfile || isCalendar ? item.id === (new URLSearchParams(location.search).get('role') || (isCalendar ? 'events' : 'tot')) ? 'text-[#544CF6] border-b-2 border-[#544CF6] pt-[2px]' : 'text-white opacity-80 hover:opacity-100' : location.pathname + location.search === item.path || location.pathname === item.path && location.search === '' && item.id === 'xat' ? 'text-[#544CF6] border-b-2 border-[#544CF6] pt-[2px]' : 'text-white opacity-80 hover:opacity-100'}
                              `}>
                              {item.label}
                          </button>)}
                  </div>
              ...
              <div className="flex items-center h-full px-4 bg-[#1a1a1a]/90 backdrop-blur-md border-l border-white/5 shadow-[-10px_0_15px_rgba(0,0,0,0.5)]">
                  <button onClick={() => navigate('/ofici/categories')} className='w-8 h-8 flex items-center justify-center rounded-[28px] bg-white/5 text-slate-400 hover:bg-sdp-theme-accent-primary hover:text-white transition-all active:scale-95 shadow-inner' title="Més accions">
