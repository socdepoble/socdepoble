...
const FloatingIndex = ({
  ...
  return (
    <>
        ...
        width: width || 320
      }} className={`absolute top-[56px] left-0 bottom-0 max-w-[85vw] bg-sdp-bg-panel border-r border-black/10 dark:border-white/10 shadow-2xl z-[900] p-6 overflow-y-auto custom-scrollbar animate-in slide-in-from-left duration-300 ${isPinned ? 'lg:shadow-none' : ''}`}>
                      {isPinned && onDragStart && <div className='w-2 cursor-col-resize absolute right-[-4px] top-0 h-full z-50 hover:bg-sdp-theme-accent-primary/20 hidden lg:block' onMouseDown={onDragStart} title="Arrossega per redimensionar" />}
                      <div className="flex items-center justify-between mb-6">
                          <h4 className='text-sm font-black uppercase tracking-widest text-sdp-theme-accent-primary flex items-center gap-2'>
                              <Book size={20} />
                              Índex
                          </h4>
                          <div className="flex items-center gap-1">
                              {onPinToggle && <button onClick={() => onPinToggle(!isPinned)} className={`p-2 rounded-full transition-colors active:scale-95 hidden lg:flex ${isPinned ? 'bg-[var(--theme-accent-primary)]/10 text-[var(--theme-accent-primary)]' : 'hover:bg-black/5 dark:hover:bg-white/5 text-stone-400'}`} title={isPinned ? "Desfixar índex" : "Fixar índex"}>
