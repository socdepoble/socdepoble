...
export const UniversalInput = React.forwardRef(({
  ...
  return <div className={`${styles['input-group']} ${className}`}>
        {label && <label htmlFor={inputId} className={styles.label}>
            {label}
          </label>}
        
        <input id={inputId} ref={ref} className={styles['input-field']} aria-invalid={!!error} aria-describedby={error ? `${inputId}-error` : undefined} {...props} />
        
        {error && <span id={`${inputId}-error`} className={styles['error-message']} role="alert">
            {error}
          </span>}
      </div>;
});
