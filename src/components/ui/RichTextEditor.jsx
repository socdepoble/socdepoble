...
const MenuBar = ({
  ...
  return (
    <div className={`flex flex-wrap items-center gap-1 border-b border-gray-200 bg-white sticky top-0 z-10 w-full shadow-sm ${minimal ? 'p-1' : 'p-2'}`}>
        ...
        <ToolbarButton onClick={() => editor.chain().focus().redo().run()} disabled={!editor.can().chain().focus().redo().run()} ariaLabel="Refer">
        ...
        <ToolbarButton onClick={() => editor.chain().focus().toggleItalic().run()} disabled={!editor.can().chain().focus().toggleItalic().run()} isActive={editor.isActive('italic')} ariaLabel="Cursiva">
          
          <Italic size={minimal ? 16 : 18} />
        </ToolbarButton>
        {!minimal && <>
              <ToolbarButton onClick={() => editor.chain().focus().toggleUnderline().run()} disabled={!editor.can().chain().focus().toggleUnderline().run()} isActive={editor.isActive('underline')} ariaLabel="Subratllat">
            
                  <UnderlineIcon size={18} />
              </ToolbarButton>
              <ToolbarButton onClick={() => editor.chain().focus().toggleStrike().run()} disabled={!editor.can().chain().focus().toggleStrike().run()} isActive={editor.isActive('strike')} ariaLabel="Ratllat">
      ...
      }} isActive={editor.isActive('link')} ariaLabel="Enllaç">
        ...
        <ToolbarButton onClick={() => editor.chain().focus().toggleOrderedList().run()} isActive={editor.isActive('orderedList')} ariaLabel="Llista Numerada">
          
          <ListOrdered size={minimal ? 16 : 18} />
        </ToolbarButton>
        <ToolbarButton onClick={() => editor.chain().focus().toggleBlockquote().run()} isActive={editor.isActive('blockquote')} ariaLabel="Cita">
          
          <Quote size={minimal ? 16 : 18} />
        </ToolbarButton>

        {onOpenHtmlDialog && <ToolbarButton onClick={onOpenHtmlDialog} ariaLabel="Incrustar HTML">
          
              <CodeXml size={minimal ? 16 : 18} />
            </ToolbarButton>}

        {!minimal && <ToolbarButton onClick={() => {
        const isActive = editor.isActive('listItem', {
          isButton: true
        });
        if (isActive) {
        ...
        isButton: true
      })} disabled={!editor.isActive('listItem')} ariaLabel="Convertir a botó">
        ...
        {!minimal && <div className="hidden sm:flex items-center gap-1">
          ...
          textAlign: 'left'
              ...
              <ToolbarButton onClick={() => editor.chain().focus().setTextAlign('center').run()} isActive={editor.isActive({
          textAlign: 'center'
        })} ariaLabel="Centrar">
...
const RichTextEditor = ({
  ...
  const editor = useEditor({
    ...
    extensions: [StarterKit.configure({
      listItem: false
    }), CustomListItem, TextAlign.configure({
      types: ['heading', 'paragraph']
    }), Image, Link.configure({
      openOnClick: !editable
    }), Underline, TextStyle, Color, RawHTMLNode],
