...
export default function ButtonTrellat({ 
  ...
  return (
    <button 
      ...
      onClick={onClick}
      disabled={disabled}
      className={`inline-flex items-center justify-center min-w-[56px] min-h-[56px] rounded-full font-semibold px-4 transition-all
                  will-change-transform focus:outline-none focus-visible:ring-2 active:scale-95 touch-manipulation disabled:opacity-50
                  ${intentColors[intent] || intentColors.primary} ${className}`}
