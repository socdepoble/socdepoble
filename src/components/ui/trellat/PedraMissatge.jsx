...
const PedraMissatge = memo(({
  ...
  return <article className={`pedra-missatge ${isOwn ? 'pedra-missatge--propi' : 'pedra-missatge--altri'}`} data-message-id={id} style={{
    // CONTRACTE STRICTE:
    ...
    contain: 'layout paint'
      ...
      <header className="pedra-missatge__meta flex items-center justify-between mb-1 opacity-70 px-2">
        {!isOwn && author?.name && <span className="pedra-missatge__autor text-xs font-bold">{author.name}</span>}
        <time className="pedra-missatge__hora text-xs" dateTime={timestamp ? new Date(timestamp).toISOString() : undefined}>
          {formatTime(timestamp)}
        </time>
      </header>

      <div className="pedra-missatge__cos" style={{
      ...
      padding: 'calc(var(--sp-space-3, 12px) * var(--lupa-scale, 1))'
        ...
        {content && <p className="pedra-missatge__text text-base" style={{
        margin: 0
      }}>
            {content}
          </p>}

        {attachments?.map((att, idx) => <figure key={`${id}-att-${idx}`} className="pedra-missatge__adjunt" style={{
        margin: '0.75rem 0 0 0',
        borderRadius: '0.75rem',
        overflow: 'hidden'
      }}>
            {att.type === 'image' && <img src={att.src} alt={att.alt || 'Imatge adjunta'} loading="lazy" decoding="async" style={{
          display: 'block',
          width: '100%',
          height: 'auto',
          maxHeight: '300px',
      ...
      {isOwn && status && <footer className="pedra-missatge__estat flex justify-end" style={{
      marginTop: '0.25rem',
      padding: '0 0.75rem',
      fontSize: '0.75px',
      opacity: 0.7
    }}>
          <span className="text-xs" aria-label={status === 'read' ? 'Llegit' : status === 'delivered' ? 'Lliurat' : 'Enviat'}>
            {status === 'read' ? '✓✓' : '✓'}
          </span>
        </footer>}
    </article>;
