...
class RhizomeDB {
  ...
  handleWorkerMessage(e) {
    ...
    if (type === 'DEBUG') {
      if (logger.debug) logger.debug(payload);
      return;
    }
    if (type === 'ERROR' && !id) {
  ...
  handleWorkerCrash(err) {
    ...
    for (const [id, callback] of pending.entries()) {
      if (typeof callback === 'function') {
        callback({
          type: 'ERROR',
          payload: 'Rhizome Worker no disponible (crashed)'
        });
      }
      pending.delete(id);
    }
  ...
  sendToWorker(type, payload, callback, timeoutMs = 15000) {
    if (!this.worker) {
      ...
      if (callback) callback({
        type: 'ERROR',
        payload: 'Worker no inicialitzat'
      });
      return;
    }
    if (!this.pendingRequests) {
    ...
    if (callback) {
      const timeoutId = setTimeout(() => {
        if (this.pendingRequests.has(id)) {
          ...
          callback({
            type: 'ERROR',
            payload: `Worker timeout (${timeoutMs}ms) per a l'operació ${type}`
          });
        }
      }, timeoutMs);
  ...
  async saveOperation(op) {
    ...
    return new Promise((resolve, reject) => {
      this.sendToWorker('SAVE_OP', op, res => {
        if (res.type === 'ERROR') reject(new Error(res.payload));else resolve();
      });
    });
  }
  async saveOperationsBatch(ops) {
    ...
    return new Promise((resolve, reject) => {
      this.sendToWorker('SAVE_OPS_BATCH', {
        ops
      }, res => {
        if (res.type === 'ERROR') reject(new Error(res.payload));else resolve();
      });
  ...
  async getOperations(docId) {
    await this.init();
    return new Promise((resolve, reject) => {
      this.sendToWorker('GET_OPS', {
        docId
      }, res => {
        if (res.type === 'ERROR') reject(new Error(res.payload));else resolve(res.payload);
      });
    });
  }
  async saveSnapshot(docId, data, lastOpId, vectorClock) {
    ...
    return new Promise((resolve, reject) => {
      this.sendToWorker('SAVE_SNAPSHOT', {
        docId,
        data,
        lastOpId,
        vectorClock,
        ...
        if (res.type === 'ERROR') reject(new Error(res.payload));else {
  ...
  async getSnapshot(docId) {
    await this.init();
    return new Promise((resolve, reject) => {
      this.sendToWorker('GET_SNAPSHOT', {
        docId
      }, async res => {
        if (res.type === 'ERROR') reject(new Error(res.payload));else {
  ...
  async getTrustScore(myDid, targetDid) {
    await this.init();
    return new Promise((resolve, reject) => {
      this.sendToWorker('GET_TRUST_SCORE', {
        myDid,
        targetDid
      }, res => {
        if (res.type === 'ERROR') reject(new Error(res.payload));else resolve(res.payload);
