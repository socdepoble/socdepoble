...
export const marketService = {
    ...
    async getMarketItems(categoryFilter = 'tot', townId = null, page = 0, pageSize = 12, isPlayground = false, _isRetry = false) {
        try {
            ...
            if (error) {
                ...
                if (isColumnError) {
                    ...
                    let changed = false;
                    if (error.message?.includes('pinned_position')) { setColumnCache('market_pinned_position', false); changed = true; }
                    if (error.message?.includes('is_pinned')) { setColumnCache('market_is_pinned', false); changed = true; }
                    if (error.message?.includes('is_playground')) { setColumnCache('market_is_playground', false); changed = true; }
                    if (error.message?.includes('fk_market_town_uuid')) { setColumnCache('market_fk_town_uuid', false); changed = true; }
                    ...
                    if (!changed) {
                       ...
                       setColumnCache('market_fk_town_uuid', false);
                    }

                    if (!_isRetry) {
                        // Retry once immediately
                        return marketService.getMarketItems(categoryFilter, townId, page, pageSize, isPlayground, true);
                    }
                }
                if (cachedData) {
