...
export async function runMigrations({
  ...
  try {
    ...
    const startIndex = checkpoint.lastMigration ? MIGRATIONS.findIndex(m => m.id === checkpoint.lastMigration) + 1 : 0;
    for (let i = startIndex; i < MIGRATIONS.length; i++) {
      const mig = MIGRATIONS[i];
      await setCheckpoint({
        lastMigration: mig.id,
        step: 0,
        startedAt: Date.now()
      });
      onProgress({
...
export async function safePurgeOldData({
  keepNamespaces = []
} = {}) {
