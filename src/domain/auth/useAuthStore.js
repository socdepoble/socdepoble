...
export const useAuthStore = create((set, get) => ({
  ...
  setSimulatedRole: role => {
    ...
    if (role) localStorage.setItem('simulatedRole', role);else localStorage.removeItem('simulatedRole');
  },
  setIsPlayground: val => {
    const {
      realUser
  ...
  setImpersonatedProfile: profile => set({
    impersonatedProfile: profile
  }),
  setActiveEntityId: id => set({
    activeEntityId: id
  }),
  // --- ACCIONS COMPLEXES ---
  ...
  adoptPersona: personaProfile => {
    ...
    const newUser = {
      ...
      email: `${personaProfile.username}@playground.local`,
      isDemo: true
    };
    set({
      user: newUser,
  ...
  switchContext: async (entityId = null) => {
    ...
    try {
      ...
      if (entityData) {
        ...
        set({
          impersonatedProfile: impersonated,
  ...
  handleAuthEvent: async (event, session) => {
    ...
    try {
      const isSimulation = isPlayground || localStorage.getItem('sb-simulation-mode') === 'true' || session?.user?.id === IAIA_ID;
      if (session?.user) {
        if (isSimulation) {
          set({
            isPlayground: false
        ...
        try {
          ...
          const {
            effectiveProfile,
            isOfficialCreator
          } = profileHealingService.protectMasterIdentity(session, profileData);
          set({
            realProfile: effectiveProfile,
