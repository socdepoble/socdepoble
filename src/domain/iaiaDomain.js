export const isIAIAOfficial = post => {
  return post.author_entity_id === 'socdepoble' || post.creator_entity_id === 'socdepoble' || post.author_name?.includes('Sóc de Poble');
};
export const isIAIAMarIA = (post, authorIdCheck) => {
  return authorIdCheck === '11111111-1111-4111-a111-000000000000' || post.author_name === 'IAIA MarIA';
};
export const isImmersiveAI = (post, authorIdCheck) => {
  return post.author_is_ai || post.is_iaia_inspired || authorIdCheck && String(authorIdCheck).startsWith('11111111-') && authorIdCheck !== '11111111-1111-4111-a111-000000000000' || ['FLASH', 'GALL', 'VIATJANT', 'SULTAN', 'MIXA', 'RATOLÍ'].some(n => post.author_name?.toUpperCase().includes(n));
};
