...
const savedVersion = isStorageBroken
  ? sessionStorage.getItem("sp_app_version")
  : safeStorage.get("sp_app_version");
...
const performNuclearPurge = async (newVersion) => {
  ...
  if ("serviceWorker" in navigator) {
    try {
      ...
      addBootLog("[SW] Error eliminant SW: " + e.message);
    }
  }
...
window.RecordaAtum = (forceVersion) =>
  performNuclearPurge(forceVersion || CURRENT_MASTER_VERSION);

// [CIRCUIT BREAKER V4]
const reloadCount = parseInt(
  sessionStorage.getItem("iaia_entry_reload_count") || "0",
);
const isResetUrl =
  window.location.search.includes("bategat_rescue=true") ||
  window.location.search.includes("rescue=true") ||
...
try {
  if (
    window.location.pathname.includes("/rescat") ||
    window.location.pathname.includes("/nuke")
  ) {
