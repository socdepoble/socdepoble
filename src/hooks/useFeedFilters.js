...
export const useFeedFilters = ({
  ...
  return useMemo(() => {
    ...
    if (contextualSearchTerm) {
      ...
      filtered = filtered.filter(post => post.content?.toLowerCase().includes(normalized) || post.author_name?.toLowerCase().includes(normalized) || post.author?.toLowerCase().includes(normalized) || post.excerpt?.toLowerCase().includes(normalized));
    }

    // 5. IAIA Portera (Cognitive Filter Km 0)
    if (isIAIAFiltering) {
      const userPrefs = {
        primary_town_id: activeTown || 1,
        // Default to current town
