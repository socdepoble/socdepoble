...
export const useGoogleAuthCalendar = currentDate => {
  ...
  const [selectedCalIds, setSelectedCalIds] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('gcal_selected_calendars') || '[]');
    } catch {
      return [];
    }
  });
  ...
  const toggleCalendar = id => {
    const newer = selectedCalIds.includes(id) ? selectedCalIds.filter(v => v !== id) : [...selectedCalIds, id];
    ...
    const fetchCalendars = async () => {
      try {
        ...
        if (res.status === 401) {
          logout();
          return;
        } // Token ha mort, forçem desconnexió cívica
