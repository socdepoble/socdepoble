...
export const useInternalCalendar = currentDate => {
  ...
  const [selectedInternalCalIds, setSelectedInternalCalIds] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('sdb_selected_internal_calendars') || '[]');
    } catch {
      return [];
    }
  });
  ...
  const toggleInternalCalendar = useCallback(id => {
    const newer = selectedInternalCalIds.includes(id) ? selectedInternalCalIds.filter(v => v !== id) : [...selectedInternalCalIds, id];
