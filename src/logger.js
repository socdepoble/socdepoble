...
function writeBuffer(buf) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(buf)); } catch (e) { /* no-op */ }
}

const defaultConfig = {
  verbose: false,
  endpoint: null,           // URL on which POST({ logs: [...] }) will be accepted
...
const logger = {
  init(options = {}) {
    ...
    startAutoFlush();
    // log d'inici
    logger.info('logger.init', { cfg: { ...cfg, endpoint: cfg.endpoint ? '[REDACTED]' : null } });
    return cfg;
  },
  ...
  captureErrors() {
    ...
    errorHandler = (ev) => {
      try {
        ...
        pushLog(makeEntry('error', { source: 'window.error', ...payload }));
      } catch (e) {}
    };
    rejectionHandler = (ev) => {
      try {
        pushLog(makeEntry('error', { source: 'unhandledrejection', reason: ev.reason }));
      } catch (e) {}
    };
  ...
  captureSWMessages(registration) {
    ...
    swMessageHandler = (ev) => {
      try {
        const data = ev.data || {};
        pushLog(makeEntry('info', { source: 'sw.message', data }));
      } catch (e) {}
    };
    ...
    if (registration) {
      try {
        registration.addEventListener && registration.addEventListener('updatefound', () => {
          pushLog(makeEntry('info', { source: 'sw.updatefound' }));
        });
      } catch (e) {}
    }
  },
  ...
  info(tag, obj = {}) { pushLog(makeEntry('info', { tag, ...obj })); },
  warn(tag, obj = {}) { pushLog(makeEntry('warn', { tag, ...obj })); },
  error(tag, obj = {}) { pushLog(makeEntry('error', { tag, ...obj })); },

  // Força enviament immediat (retorna Promise<boolean>)
  flushNow() { return sendBuffered(); },

  // Neteja local del buffer
  purgeLocal() { writeBuffer([]); },

  // Obtenir buffer actual (per UI)
  getBuffer() { return readBuffer(); },
