...
const Chrome145Report = () => {
  ...
  return <div className="min-h-screen bg-black text-white p-6 lg:p-12 animate-in">
            <div role="region" aria-label="Capçalera de Secció" className="flex items-center justify-between mb-12">
          ...
          if (window.history.length > 1) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                <MetricCard icon={<Zap className="text-yellow-400" />} label="First Paint" value={`${metrics.firstPaint}ms`} status="EXCEL·LENT" />
                <MetricCard icon={<Clock className="text-indigo-400" />} label="FCP" value={`${metrics.firstContentfulPaint}ms`} status="OPTIMITZAT" />
                <MetricCard icon={<Activity className="text-rose-400" />} label="DOM Interactive" value={`${metrics.domInteractive}ms`} status="FLUID" />
                <MetricCard icon={<Globe className="text-cyan-400" />} label="Soft Navigations" value={metrics.softNavigations} status="DETECTADES" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    <section className="bg-white/5 border border-white/10 rounded-[28px] p-8">
                        ...
                        {metrics.renderBlockingResources.length > 0 ? <div className="space-y-4">
                                {metrics.renderBlockingResources.map((res, i) => <div key={i} className="flex items-center justify-between p-4 bg-white/5 rounded-[28px] border border-white/5">
                                        <div className="flex items-center gap-4">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <p className="text-xs text-slate-500 uppercase">Bloqueig Render</p>
                                        </div>
                                    </div>)}
                            </div> : <p className="text-slate-500">No s'han detectat recursos bloquejants crítics. Netedat absoluta.</p>}
                    </section>
                    ...
                    <section className="bg-white/5 border border-white/10 rounded-[28px] p-8">
                        ...
                        <div className="p-6 bg-indigo-500/5 border border-indigo-500/20 rounded-[28px]">
                            ...
                            <a href="https://github.com/ChromeDevTools/chrome-devtools-mcp" target="_blank" className="inline-flex items-center gap-2 text-indigo-400 font-bold uppercase text-xs hover:underline">
                ...
                <aside className="space-y-8">
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-[28px] p-8 text-center">
                        ...
                        <p className="text-xs text-slate-400 uppercase mt-2">Disponibilitat Bategada</p>
                    </div>
                </aside>
            </div>
        </div>;
...
const MetricCard = ({
  ...
  status
        ...
        <div className="flex items-center gap-3 mb-4">
            {icon}
            <span className="text-xs font-black uppercase text-slate-500 tracking-wider">{label}</span>
        </div>
