...
const Register = () => {
  ...
  const handleVerifyOtp = useCallback(async (e, codeToVerify = null) => {
    ...
    try {
      ...
      const {
        user: verifiedUser
      } = await authService.verifyOtp(formattedPhone, code);
      if (verifiedUser) {
  ...
  const languages = [{
    ...
    label: "GAL",
    flag: "🐙"
  } // Updated per USER request: Galician instead of French
  ];
  return (
    <div className="flex flex-col md:flex-row w-full min-h-[100dvh] bg-theme-base relative overflow-hidden font-sans">
        ...
        <div className='hidden md:flex md:w-1/2 lg:w-[55%] bg-sdp-theme-accent-primary-faint relative flex-col items-center justify-center p-8 lg:p-12 border-r border-sdp-border-master overflow-hidden'>
          ...
          <div className="relative z-10 w-full max-w-xl mb-12">
            <BrandLogo className='w-full h-auto object-contain drop-shadow-2xl text-sdp-theme-accent-primary' />
          </div>
          
          {/* IAIA Guide per a escriptori (Més gran i premium) */}
          <div className='relative z-10 w-full max-w-xl bg-theme-base/80 backdrop-blur-xl border border-sdp-theme-accent-primary-muted rounded-[32px] p-8 flex gap-8 items-start shadow-2xl hover:-translate-y-1 transition-transform'>
            <div className='w-24 h-24 shrink-0 rounded-full bg-sdp-theme-accent-primary-faint flex items-center justify-center shadow-inner overflow-hidden border-4 border-white dark:border-gray-800'>
               <img src="/uploads/avatars/iaia_comic_matriarch.png" alt="IAIA" className="w-[110%] h-[110%] object-cover object-top" />
            </div>
            <div className="flex-1 pt-2">
               <h3 className='font-black text-sdp-theme-accent-primary text-xl mb-3 uppercase tracking-tight'>IAIA Guia</h3>
        ...
        <div className="w-full md:w-1/2 lg:w-[45%] flex-1 flex flex-col relative overflow-y-auto custom-scrollbar pb-12 md:pb-0">
          ...
          <div className='absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-sdp-theme-accent-secondary to-sdp-theme-accent-primary opacity-80 md:hidden'></div>
          ...
          <div className="w-full flex-1 flex flex-col px-6 md:px-12 lg:px-20 pt-6 md:pt-12 pb-8 animate-in-up max-w-md md:max-w-2xl mx-auto md:justify-center relative">
            ...
            <div className='flex justify-center gap-1.5 mb-8 bg-theme-panel md:bg-transparent border border-sdp-border-master md:border-none p-1.5 md:p-0 rounded-full z-20 mx-auto'>
                {languages.map(lang => <button key={lang.code} className={`px-4 py-2 rounded-full transition-all text-[11px] font-black uppercase tracking-widest ${language === lang.code ? "bg-[var(--theme-accent-primary)] text-white shadow-sm" : "text-gray-400 hover:text-theme-text"}`} onClick={() => {
            ...
            <div role="region" aria-label="Capçalera de Secció" className="flex flex-col items-center pt-2 pb-6 md:pb-12 md:hidden">
              <BrandLogo className='w-[280px] max-w-[80vw] h-auto object-contain mb-8 transition-all text-sdp-theme-text' />

              {/* IAIA Guide Mobile */}
              <div className='w-full bg-sdp-theme-accent-primary-faint border border-sdp-theme-accent-primary-muted rounded-2xl p-5 flex gap-5 items-start shadow-sm mt-4'>
                 <div className='w-16 h-16 shrink-0 rounded-full bg-sdp-theme-accent-primary-faint flex items-center justify-center shadow-inner overflow-hidden border border-sdp-theme-accent-primary-muted' onClick={() => hapticService.batec()}>
                     <img src="/uploads/avatars/iaia_comic_matriarch.png" alt="IAIA" className="w-[110%] h-[110%] object-cover object-top" />
                 </div>
                 <div className="flex-1 pt-1">
                     <h3 className='font-black text-sdp-theme-accent-primary text-base mb-1.5 uppercase tracking-tight'>IAIA Guia</h3>
          ...
          {step !== "welcome" && <div className="mb-6 bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4 flex gap-4 items-start shadow-sm mx-1 animate-in-up" style={{
            ...
            animationFillMode: 'both'
                ...
                <div className="flex-1 pt-0.5">
                  ...
                  <div className="mt-3">
                    <Link to="/el-projecte" className='inline-flex items-center gap-1.5 bg-sdp-theme-accent-primary text-white px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest hover:brightness-110 transition-all shadow-sm active:scale-95 text-xs'>
          ...
          {/* STEP 1: IDENTITY */}
          {step === "identity" && <div className="flex flex-col gap-6 animate-fade-in-right shrink-0 pb-8">
              
              <button onClick={signInWithGoogle} disabled={loading} type="button" className='w-full h-16 bg-theme-panel border-2 border-sdp-border-master rounded-[20px] flex items-center justify-center gap-3 hover:bg-sdp-hover-overlay transition-all active:scale-[0.98] shadow-sm'>
              ...
              {ENABLE_MANUAL_REGISTRATION && <>
                  <div className="relative flex items-center justify-center py-2">
                      <div className="absolute inset-0 flex items-center">
                        <div className='w-full border-t border-sdp-border-master'></div>
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <input id="reg-name" name="full_name" type="text" placeholder="El teu nom i cognoms..." value={fullName} onChange={e => {
                  setFullName(e.target.value);
                  if (e.target.value.length === 3) hapticService.batec();
                }} autoComplete="name" required className={`w-full h-16 bg-theme-panel border-2 rounded-[20px] px-5 text-theme-text font-bold text-xl outline-none transition-all placeholder:text-gray-500 placeholder:font-normal
                          ${fullName && !isNameValid ? "border-red-400 focus:border-red-500 bg-red-400/10" : isNameValid ? "border-green-500 focus:border-green-600 bg-green-500/10" : "border-[var(--border-master)] focus:border-[var(--theme-accent-primary)] focus:bg-[var(--theme-accent-primary-faint)]"}`} />
                  
                  </div>

                  <div className="pt-2">
                    <button className={`w-full h-16 rounded-[20px] flex items-center justify-center text-white font-black uppercase tracking-widest transition-all active:scale-[0.98] ${!isNameValid ? "bg-gray-300 opacity-50 cursor-not-allowed" : "bg-[var(--theme-accent-primary)] hover:opacity-90 shadow-[0_0_15px_rgba(255,107,0,0.3)]"}`} disabled={!isNameValid} onClick={() => {
                  hapticService.batec();
                  setStep("town");
                }}>
          ...
          {/* STEP 2: TOWN */}
          {step === "town" && <div className="flex flex-col gap-6 animate-fade-in-right flex-1">
              <div className="space-y-3">
                ...
                <button type="button" onClick={() => setShowTownPicker(true)} className='w-full flex items-center justify-between px-5 py-5 bg-theme-panel border-2 border-sdp-border-master rounded-[20px] hover:border-sdp-theme-accent-primary hover:bg-sdp-theme-accent-primary-faint transition-all active:scale-[0.98] shadow-sm'>
                  
                  <div className="flex items-center gap-4">
                    <div className='w-12 h-12 bg-theme-base shadow-sm border border-sdp-border-master rounded-2xl flex items-center justify-center shrink-0'>
                      <MapPin size={26} className='text-sdp-theme-accent-primary' />
                    </div>
                  ...
                  <div className='bg-sdp-hover-overlay p-3 rounded-2xl text-gray-400'>
              ...
              <div className="pt-4 flex gap-3">
                <button className='h-16 px-6 rounded-[20px] font-bold text-theme-text opacity-70 bg-theme-panel border border-sdp-border-master hover:bg-sdp-hover-overlay transition-colors' onClick={() => setStep("identity")}>
                  Enrere
                </button>
                <button className={`flex-1 h-16 rounded-[20px] flex items-center justify-center gap-2 text-white font-black uppercase tracking-widest transition-all active:scale-[0.98] ${!selectedTown ? "bg-gray-300 cursor-not-allowed" : "bg-[#0ea5e9] hover:bg-[#0284c7] shadow-lg shadow-sky-500/30"}`} disabled={!selectedTown} onClick={() => {
                hapticService.batec();
                setStep("connection");
              }}>
          ...
          {/* STEP 3: CONNECTION */}
          {step === "connection" && <form onSubmit={handleRegister} className="flex flex-col gap-6 animate-fade-in-right flex-1">
              <div className="space-y-3">
                ...
                }} autoComplete="tel" inputMode="tel" required className={`w-full h-16 bg-theme-panel border-2 rounded-[20px] pl-24 pr-5 text-theme-text font-bold text-2xl outline-none transition-all placeholder:text-gray-600 placeholder:font-normal tracking-wide
                      ${phone && !isPhoneValid ? "border-red-400 focus:border-red-500 bg-red-400/10" : isPhoneValid ? "border-green-500 focus:border-green-600 bg-green-500/10" : "border-[var(--border-master)] focus:border-[var(--theme-accent-primary)] focus:bg-[var(--theme-accent-primary-faint)]"}`} />
                  
                </div>
              </div>

              <div className="pt-4 flex gap-3">
                  {mode === "register" && <button type="button" className='h-16 px-6 rounded-[20px] font-bold text-theme-text opacity-70 bg-theme-panel border border-sdp-border-master hover:bg-sdp-hover-overlay transition-colors' onClick={() => setStep("town")}>
                  
                      Enrere
                    </button>}
                  <button type="submit" className={`flex-1 h-16 rounded-[20px] flex items-center justify-center gap-2 text-white font-black uppercase tracking-widest transition-all active:scale-[0.98] ${!isPhoneValid ? "bg-gray-300 cursor-not-allowed opacity-50" : "bg-[var(--theme-accent-primary)] hover:opacity-90 shadow-[0_0_15px_rgba(255,107,0,0.3)]"}`} disabled={loading || !isPhoneValid}>
          ...
          {/* STEP 4: VERIFY */}
          {step === "verify" && <form onSubmit={handleVerifyOtp} className="flex flex-col gap-6 animate-fade-in-right flex-1">
              ...
              <div className="space-y-4">
                ...
                <input id="otp-input-reg" name="otp_code" type="text" placeholder="123456" value={otp} onChange={e => setOtp(e.target.value)} autoComplete="one-time-code" inputMode="numeric" maxLength={6} required className='w-full h-20 bg-theme-panel border-2 border-sdp-border-master focus:border-sdp-theme-accent-primary focus:bg-sdp-theme-accent-primary-faint rounded-[20px] text-center text-4xl font-black text-theme-text tracking-[0.5em] outline-none transition-all placeholder:text-gray-600 shadow-sm' />
                
              </div>

              <div className="mt-auto pt-6 space-y-4">
                <button type="submit" className={`w-full h-16 rounded-[20px] flex items-center justify-center gap-2 text-white font-black uppercase tracking-widest transition-all active:scale-[0.98] ${otp.length < 6 || loading ? "bg-gray-300 opacity-50 cursor-not-allowed" : "bg-[var(--theme-accent-primary)] hover:opacity-90 shadow-[0_0_15px_rgba(255,107,0,0.3)]"}`} disabled={otp.length < 6 || loading} onClick={() => hapticService.batec()}>
                ...
                <div className="text-center pt-4 pb-2 space-y-4">
                  <button type="button" className='text-base font-bold text-gray-400 hover:text-theme-text underline underline-offset-4 decoration-2 decoration-sdp-border-master transition-colors' disabled={resendCountdown > 0} onClick={e => {
                  ...
                  <button type="button" onClick={() => setStep("connection")} className='text-sm font-black text-gray-400 hover:text-sdp-theme-accent-primary uppercase tracking-widest transition-colors bg-theme-panel border border-sdp-border-master px-4 py-2 rounded-full'>
                        Canviar Telèfon
                    </button>
          ...
          {step === "welcome" && <div className="text-center py-12 space-y-6 animate-in-up flex-1 flex flex-col items-center justify-center">
          ...
          <div className="mt-8">
            <div className="text-center text-xs text-gray-400 mb-6 font-medium leading-relaxed px-4">
              ...
              <br />
              <Link to="/legal/aviso-legal" className="underline hover:text-theme-text transition-colors mt-1 inline-block">Avisos Legals</Link>
            </div>
            
            <div className='text-center text-lg bg-theme-panel rounded-2xl p-4 border border-sdp-border-master'>
              <span className="text-gray-400 font-medium">Ja tens compte?</span>{" "}
              <Link to="/login" className='text-sdp-theme-accent-primary font-black hover:underline tracking-wide ml-1'>
                Entra ara
              </Link>
            </div>

            <div className="text-center mt-6 text-gray-300 font-bold text-xs uppercase tracking-widest">
              {APP_VERSION}
            </div>
          </div>
