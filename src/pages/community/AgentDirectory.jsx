...
const AgentDirectory = () => {
    ...
    return (
        <div className="search-discover-page min-h-screen bg-theme-base overflow-y-auto">
            <SEO 
                title="Agents IA | Sóc de Poble" 
                description="Directori oficial dels Agents Intel·ligents de Sóc de Poble." 
                url="/agents" 
            />
            <SearchNavBar 
                query={query}
            ...
            <div ref={containerRef} className="flex-1 w-full pt-4 pb-32 pb-safe animate-in fade-in slide-in-from-bottom-4 duration-500">
                <UniversalGridWrapper viewMode={viewMode}>
                    {agents.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-center text-theme-muted font-bold text-lg">
                        ...
                        <UniversalGridRow viewMode={viewMode} columnCount={columnCount}>
                            {agents.map((agent, index) => (
                                <UniversalCard
                                    ...
                                    item={agent}
                                    title={agent.name}
                                    subtitle={agent.tag || agent.role || 'Agent IA'}
                                    avatarSrc={agent.avatar_url}
                                    avatarName={agent.name}
                                    avatarRole="agent"
                                    excerpt={agent.short_bio || (agent.systemPrompt && agent.systemPrompt.slice(0, 110) + '...')}
                                    viewMode={viewMode}
                                    variant="post"
                                    onClick={() => navigate(`/gent/${agent.id}`)}
                                />
                            ))}
                        </UniversalGridRow>
                    )}
