...
const AgentDirectory = () => {
  ...
  const { viewMode, columnCount, containerRef } = useViewMode('agents_directory_view_mode', 'grid');
  ...
  return (
    <div className="search-discover-page min-h-screen bg-white overflow-y-auto">
        ...
        <div ref={containerRef} className="flex-1 w-full pt-4 pb-32 pb-safe animate-in fade-in slide-in-from-bottom-4 duration-500">
            <UniversalGridWrapper viewMode={viewMode}>
                {agents.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center text-gray-500 font-bold text-lg">
                      NO S'HA TROBAT CAP AGENT
                  </div>
                ) : (
                  <UniversalGridRow viewMode={viewMode} columnCount={columnCount}>
                      {agents.map((agent, index) => (
                        <UniversalCard key={agent.id} item={agent} title={agent.name} subtitle={agent.tag || agent.role || 'Agent IA'} avatarSrc={agent.avatar_url} avatarName={agent.name} avatarRole="agent" excerpt={agent.short_bio || agent.systemPrompt && agent.systemPrompt.slice(0, 110) + '...'} viewMode={viewMode} variant="post" onClick={() => navigate(`/gent/${agent.id}`)} />
                      ))}
                  </UniversalGridRow>
                )}
