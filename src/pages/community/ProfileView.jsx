...
const generateUsernameFromName = name => {
  ...
  return name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '') // elimina accents
...
const ProfileView = () => {
  ...
  const {
    winningProposal
  } = useTownProposals(profile?.is_town ? profile?.raw_town_id : null);
  ...
  useEffect(() => {
    ...
    import('../../app/config/agentsMap').then(({
      AGENTS_MAP
    }) => {
  ...
  useEffect(() => {
    if (isOwnProfile && !id && myProfile?.id) {
      navigate(`/gent/${myProfile?.username || myProfile.id}`, {
        replace: true
      });
    }
      ...
      try {
        ...
        if (isOwnProfile && myProfile) {
          ...
          if (id.startsWith('11111111-') || id.startsWith('SYSTEM_')) {
            const {
              AGENTS_MAP
            } = await import('../../app/config/agentsMap');
            ...
            if (foundTown) {
              ...
              if (isAjuntamentPath) {
                targetProfile = {
                  ...
                  is_entity: true,
                  is_town: false,
                  // Tractat com entitat normal, no filtre de sistema
                  cover_position_y: 50
                };
              } else {
                targetProfile = {
  ...
  const handleStudioFileSelect = async (e, type) => {
    ...
    try {
      ...
      if (e.target.value && typeof e.target.value === 'string' && e.target.value.startsWith('icon:')) {
        ...
        const {
          error: uploadError
        } = await supabaseService.supabase.storage.from('avatars').upload(filePath, file);
        if (uploadError) throw new Error("Error pujant imatge.");
        const {
          data
        } = supabaseService.supabase.storage.from('avatars').getPublicUrl(filePath);
        updates[`${type}_url`] = data.publicUrl;
      }
      const {
        error: dbError
      } = await supabaseService.supabase.from('users').update(updates).eq('id', profile.id);
  ...
  if (loading) return <div className={`flex flex-col items-center justify-center min-h-screen ${bgColor} ${textColor}`}>
            <Loader2 className="animate-spin text-orange-500 mb-4" size={48} />
            <span className="font-black uppercase tracking-[0.3em] text-xs">Cercant les dades al Mas...</span>
        </div>;
  if (error) return <div className={`flex flex-col items-center justify-center min-h-screen ${bgColor} ${textColor} p-6`}>
  ...
  return (
    <div className="flex flex-col w-full h-screen bg-theme-base overflow-hidden">
              ...
              {showTopBtn && <button onClick={scrollToTop} className='fixed bottom-24 right-6 md:bottom-10 md:right-10 w-12 h-12 md:w-14 md:h-14 bg-sdp-theme-accent-primary hover:bg-sdp-theme-accent-primary-hover text-white rounded-full flex items-center justify-center shadow-[0_10px_30px_rgba(255,107,0,0.5)] transition-all animate-in fade-in zoom-in z-overlay' title="Torna a dalt ràpidament">
        ...
        handleStudioFileSelect({
          target: {
            files: [media]
          }
        }, type);
      }} />
              {isOwnProfile && profile && <ProfileSettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} profile={profile} onProfileUpdate={updates => setProfile(prev => ({
        ...prev,
        ...updates
      }))} />}
          </div>
