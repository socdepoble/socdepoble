...
const UsersDirectory = () => {
  ...
  const {
    ...
    columnCount,
    containerRef
  } = useViewMode('users_directory_view_mode', 'grid');
  ...
  return (
    <div className="directory-page bg-theme-base min-h-screen pb-20">
              <SEO title="Veïnatge | Sóc de Poble" description="Persones que fan poble. Directori oficial de veïns de la comunitat Sóc de Poble." url="/users" />
              {/* Header Master Blindat v9.4.0 */}
              <div role="region" aria-label="Capçalera de Secció" className='h-16 flex items-center px-4 bg-theme-base border-b border-sdp-border-master sticky top-0 z-30'>
                  <button className='text-sdp-theme-text mr-4 hover:text-sdp-theme-accent-primary transition-colors' onClick={() => navigate(-1)}>
                      <ArrowLeft size={24} />
                  </button>
                  <div className="flex-1">
                      <h1 className='text-lg font-black text-sdp-theme-accent-primary uppercase tracking-widest m-0'>Veïnatge</h1>
                      <p className="text-xs text-theme-muted uppercase font-bold tracking-tighter m-0">Persones que fan poble</p>
                  </div>
              </div>
              ...
              <div className="px-4 py-4 bg-theme-base border-b border-theme-border">
                  <div className="flex items-center gap-2 p-3 bg-surface-var border border-theme-border/50 rounded-2xl">
                      <Users size={16} className='text-sdp-theme-accent-primary' />
                      <span className='font-bold text-xs uppercase tracking-widest text-sdp-theme-text'>
              ...
              <div ref={containerRef} className="flex-1 w-full pt-6">
                  <UniversalGridWrapper viewMode={viewMode}>
                      {filteredItems.length === 0 ? <div className="flex flex-col items-center justify-center p-12 opacity-50 bg-theme-panel/30 rounded-2xl border border-white/5 mx-auto max-w-sm mt-8">
                              ...
                              {filteredItems.map(item => <UniversalCard key={item.id} item={item} title={item.full_name || item.name || 'Anònim'} subtitle={`${item.ofici || item.role || item.type || 'Veí'} • ${item.town_name || item.primary_town || 'Senzillament rural'}`} avatarSrc={item.avatar_url} avatarName={item.full_name || item.name} avatarRole={item.role || 'user'} excerpt={item.bio || item.description || 'Viu la vida al poble.'} viewMode={viewMode} variant="post" onClick={() => navigate(`/gent/${item.id}`)} />)}
                          </UniversalGridRow>}
