...
const UsersDirectory = () => {
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
        ...
        <div role="region" aria-label="Capçalera de Secció" className='h-16 flex items-center px-4 bg-white border-b border-gray-200 sticky top-0 z-30'>
            <button className='text-gray-900 mr-4 hover:text-orange-500 transition-colors' onClick={() => navigate(-1)}>
                <ArrowLeft size={24} />
            </button>
            <div className="flex-1">
                <h1 className='text-lg font-black text-orange-500 uppercase tracking-widest m-0'>Veïnatge</h1>
                <p className="text-xs text-gray-500 uppercase font-bold tracking-tighter m-0 mt-0.5">Persones que fan poble</p>
            </div>
        </div>
        ...
        <div className="px-4 py-4 bg-white border-b border-gray-200">
            <div className="flex items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-2xl">
                ...
                <span className='font-bold text-xs uppercase tracking-widest text-gray-900'>
            ...
            <UniversalGridWrapper viewMode={viewMode}>
                {filteredItems.length === 0 ? (
                  <div className="flex flex-col items-center justify-center p-12 bg-gray-50 rounded-2xl border border-gray-200 mx-auto max-w-sm mt-8">
                      ...
                      <p className="font-sans text-xs font-bold tracking-[0.2em] uppercase text-center text-gray-500 m-0">No hi ha veïns visibles.</p>
                  </div>
                ) : (
                  <UniversalGridRow viewMode={viewMode} columnCount={columnCount}>
                      {filteredItems.map(item => (
                        <UniversalCard key={item.id} item={item} title={item.full_name || item.name || 'Anònim'} subtitle={`${item.ofici || item.role || item.type || 'Veí'} • ${item.town_name || item.primary_town || 'Senzillament rural'}`} avatarSrc={item.avatar_url} avatarName={item.full_name || item.name} avatarRole={item.role || 'user'} excerpt={item.bio || item.description || 'Viu la vida al poble.'} viewMode={viewMode} variant="post" onClick={() => navigate(`/gent/${item.id}`)} />
                      ))}
                  </UniversalGridRow>
                )}
