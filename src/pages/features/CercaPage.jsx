    ...
    return (
        <span>
            {parts.map((part, i) => 
                ...
                <span key={i} className="bg-[var(--theme-accent-primary)] text-white dark:bg-yellow-500 dark:text-black font-bold px-0.5 rounded-sm">{part}</span> : 
...
export default function CercaPage() {
    ...
    return (
        <div className="search-discover-page min-h-screen bg-theme-bg flex flex-col">
            <SEO
                title={query ? `Cerca: ${query}` : 'Cerca Universal'}
                description="Explora tot l'ecosistema de Sóc de Poble: persones, entitats, pobles, esdeveniments i arxiu documental."
                keywords="cerca, buscador, pobles, persones, arxiu, iaia"
            />
            
            <SearchNavBar
                query={query}
                setQuery={setQuery}
                placeholder="Busca al sistema..."
                onClear={clearSearch}
                customIcon={<Search className="text-[var(--theme-accent-primary)]" size={20} />}
                inputRef={inputRef}
            />

            <div className="filter-chips-container w-full overflow-x-auto no-scrollbar border-b border-[var(--theme-border)] bg-theme-bg sticky top-[60px] z-20 shadow-sm">
            ...
            <div className="search-content pt-4 flex-1 w-full max-w-4xl mx-auto px-4 pb-20">
                {isSearching ? (
                    <div className="search-loading flex flex-col items-center justify-center p-12 text-theme-muted">
                    ...
                    <>
                        {/* IAIA INTENT ROUTER */}
                        {query && (
                            <div className="intent-router-suggestion animate-in fade-in slide-in-from-top-4 mb-4">
                                {(query.toLowerCase().includes('gana') || query.toLowerCase().includes('dinar') || query.toLowerCase().includes('recepta')) ? (
                                    <div className="universal-card bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/30 dark:to-orange-800/30 border border-orange-200 dark:border-orange-800 p-4 rounded-3xl flex items-center gap-4 cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate('/tools/recipe')}>
                                    ...
                                    <div className="universal-card bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/30 border border-blue-200 dark:border-blue-800 p-4 rounded-3xl flex items-center gap-4 cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate('/ia')}>
                                    ...
                                    <div className="universal-card bg-gradient-to-r from-emerald-50 to-emerald-100 dark:from-emerald-900/30 dark:to-emerald-800/30 border border-emerald-200 dark:border-emerald-800 p-4 rounded-3xl flex items-center gap-4 cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate('/tools/diccionari')}>
                        ...
                        {searchInsights && (
                            <div className="semantic-insight-card bg-theme-panel rounded-[32px] p-5 shadow-sm border border-[var(--theme-border)] mb-6 animate-in fade-in slide-in-from-bottom-4">
                                <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-[10px] bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-400 font-bold px-2 py-0.5 rounded-full inline-block mt-0.5">Insight Actiu</div>
                                    </div>
                                </div>
                        ...
                        {!isEmpty ? (
                            <div className="search-results-container flex flex-col gap-6">
                                {filters.filter(f => f.id !== 'tots').map(filter => {
                                    ...
                                    switch (filter.id) {
                                        case 'gent':
                                            ...
                                            RenderItem = ({ item }) => (
                                                <div className="bg-theme-card rounded-[24px] p-3 flex items-center gap-4 active:scale-[0.98] transition-transform select-none cursor-pointer shadow-sm hover:shadow-md border border-[var(--theme-border)]" onClick={() => navigate(`/gent/${item.id}`)}>
                                                    ...
                                                    <div className="flex flex-col flex-1 min-w-0">
                                                        ...
                                                        <span className="text-[14px] text-theme-muted font-medium truncate"><HighlightText text={item.role || 'Foraster'} highlight={query} /> {item.primary_town ? <span>• <HighlightText text={item.primary_town} highlight={query} /></span> : ''}</span>
                                                    </div>
                                                </div>
                                            );
                                            break;
                                        case 'pobles':
                                            ...
                                            RenderItem = ({ item }) => (
                                                <div className="bg-theme-card rounded-[24px] p-3 flex items-center gap-4 active:scale-[0.98] transition-transform select-none cursor-pointer shadow-sm hover:shadow-md border border-[var(--theme-border)]" onClick={() => navigate(`/pobles/${item.id}`)}>
                                                    ...
                                                    <div className="flex flex-col flex-1 min-w-0">
                                                        ...
                                                        <span className="text-[14px] text-theme-muted font-medium truncate"><HighlightText text={item.comarca} highlight={query} /> {item.province ? <span>• <HighlightText text={item.province} highlight={query} /></span> : ''}</span>
                                                    </div>
                                                </div>
                                            );
                                            break;
                                        case 'esdeveniments':
                                            ...
                                            RenderItem = ({ item }) => (
                                                <div className="bg-[var(--color-terracotta)] text-white rounded-[24px] p-4 flex items-center gap-4 active:scale-[0.98] transition-transform select-none cursor-pointer shadow-sm hover:shadow-md hover:brightness-110" onClick={() => navigate('/pobles', { state: { initialTab: 'esdeveniments' } })}>
                                                    ...
                                                    <div className="flex flex-col flex-1 min-w-0">
                                                        ...
                                                        <span className="text-[14px] text-white/80 font-medium truncate"><HighlightText text={item.location} highlight={query} /> • {new Date(item.date).toLocaleDateString('ca-ES', { day: 'numeric', month: 'long' })}</span>
                                                    </div>
                                                </div>
                                            );
                                            break;
                                        case 'entitats':
                                            ...
                                            RenderItem = ({ item }) => (
                                                <div className="bg-theme-card rounded-[24px] p-3 flex items-center gap-4 active:scale-[0.98] transition-transform select-none cursor-pointer shadow-sm hover:shadow-md border border-[var(--theme-border)]" onClick={() => navigate(`/empresa/${item.id}`)}>
                                                    <Avatar src={item.avatar_url} role={item.type} name={item.name} size={48} />
                                                    <div className="flex flex-col flex-1 min-w-0">
                                                        <span className="text-[17px] font-bold text-theme-text truncate"><HighlightText text={item.name} highlight={query} /></span>
                                                        <span className="text-[14px] text-theme-muted font-medium truncate capitalize">{item.type} {item.town_name ? `• ${item.town_name}` : ''}</span>
                                                    </div>
                                                </div>
                                            );
                                            break;
                                        case 'arxiu':
                                            ...
                                            RenderItem = ({ item }) => (
                                                <div className="bg-theme-panel rounded-[24px] p-4 flex items-start gap-4 active:scale-[0.98] transition-transform select-none cursor-pointer shadow-sm hover:shadow-md border border-[var(--theme-border)]" onClick={() => window.open(item.link, '_blank')}>
                                                    <div className="w-12 h-12 rounded-full bg-[var(--color-orange-vibrant)]/10 text-[var(--color-orange-vibrant)] flex items-center justify-center shrink-0">
                                                        <Link2 size={24} />
                                                    </div>
                                                    <div className="flex flex-col flex-1 min-w-0 pt-1">
                                                        <span className="text-[16px] font-bold text-theme-text line-clamp-2 leading-tight mb-1"><HighlightText text={item.title} highlight={query} /></span>
                                        ...
                                        <section key={filter.id} className="animate-in fade-in slide-in-from-bottom-4">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                {sectionResults.map(item => (
                                                    <RenderItem key={item.id || item.uuid || item._id} item={item} />
                                                ))}
                                            </div>
                                        </section>
                                    );
                            ...
                            <div className="flex flex-col items-center justify-center text-center p-12 bg-theme-panel rounded-[32px] border border-[var(--theme-border)] mt-8 animate-in fade-in">
                                ...
                                {activeFilter !== 'tots' && (
                                    <button onClick={() => setActiveFilter('tots')} className="mt-6 px-6 py-2 bg-[var(--theme-accent-primary)] text-white rounded-full font-bold shadow-md hover:bg-orange-600 transition-colors">
                                        Buscar a tot arreu
                                    </button>
                                )}
                            </div>
                ...
                {isEmpty && (
                    <div className="empty-state-suggestions mt-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="mb-8">
                            <h4 className="text-sm font-black text-theme-muted uppercase tracking-wider mb-4 px-2">Cerques Populars</h4>
                            <div className="flex flex-wrap gap-2">
                                {popularSearches.map(s => (
                                    <button key={s} className="flex items-center gap-2 px-4 py-2 rounded-full bg-theme-panel border border-[var(--theme-border)] text-sm font-medium text-theme-text hover:bg-[var(--theme-surface)] hover:scale-105 transition-all shadow-sm" onClick={() => setQuery(s)}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <button 
                                ...
                                className="universal-card text-left p-6 flex flex-col items-center sm:flex-row sm:items-start gap-5 hover:shadow-lg transition-all group overflow-hidden relative"
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="font-black text-xl sm:text-2xl text-theme-text mb-2 group-hover:text-[var(--theme-accent-primary)] transition-colors">
                                        L'Equip Sintètic
                                    </h3>
