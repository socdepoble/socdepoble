...
const MedicationConfirm = () => {
  ...
  if (status === 'confirmed') {
    return <div className="fixed inset-0 bg-[#0a0a0a] z-50 flex flex-col items-center justify-center text-white p-6">
  ...
  return <div className="fixed inset-0 bg-[#0a0a0a] z-50 flex flex-col justify-between text-white p-6 pb-12">
      ...
      <div className="flex flex-col gap-4">
        <button onClick={handleConfirm} className="w-full bg-green-500 hover:bg-green-400 text-white rounded-[24px] py-6 text-2xl font-bold transition-all transform active:scale-95 shadow-[0_0_30px_rgba(34,197,94,0.3)]">
          JA L'HE PRESA
        </button>
        <button onClick={handleSnooze} className="w-full bg-gray-800 hover:bg-gray-700 text-white rounded-[24px] py-6 text-xl font-medium transition-all transform active:scale-95">
          Ajorna 10 minuts
        </button>
      </div>
    </div>;
