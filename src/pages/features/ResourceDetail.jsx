...
const StatusLoader = ({
  message
        ...
        <p className="text-xs font-black uppercase tracking-[5px] text-white animate-pulse">{message}</p>
    </div>;
...
const ResourceDetail = () => {
  ...
  useEffect(() => {
    const fetchResource = async () => {
      ...
      try {
        ...
        let found = MOCK_FEED.find(p => p.id?.toString() === id) || MOCK_MARKET_ITEMS.find(i => i.id?.toString() === id);
        if (found) {
          setResource({
            ...found,
            is_legacy: true,
  ...
  if (!resource) return <div className="min-w-full min-h-[100dvh] bg-black flex flex-col items-center justify-center p-6 text-center">
  ...
  return <div className="w-full min-h-[100dvh] bg-black text-white relative font-sans overflow-x-hidden selection:bg-white selection:text-black">
            <SEO title={`${resource.display_title} | Arxiu Sóc de Poble`} description={resource.display_content?.substring(0, 160)} image={resource.cover_image} />
            ...
            <div role="region" aria-label="Capçalera de Secció" className="relative w-full h-[65vh] md:h-[80vh] flex flex-col justify-end p-6 md:p-16 overflow-hidden rounded-b-3xl">
                ...
                <div className="absolute inset-0 z-0 bg-cover bg-center transition-transform duration-1000 scale-105" style={{
        ...
        filter: 'brightness(0.5) contrast(1.1)'
                ...
                <div className="absolute top-6 left-6 right-6 z-20 flex items-center justify-between">
                    <button onClick={() => navigate(-1)} className="flex items-center justify-center w-12 h-12 rounded-full bg-black/30 backdrop-blur-md border border-white/10 hover:bg-white/10 transition-colors">
                    ...
                    <div className="flex gap-3">
                        ...
                        <ShareHub title={resource.display_title} text={resource.display_content} url={window.location.href} />
                ...
                <div className="relative z-10 max-w-4xl animate-slide-up">
                    <div className="flex flex-wrap items-center gap-3 mb-6">
                        {resource.source_domain && <span className="px-4 py-1.5 text-xs font-black uppercase tracking-[3px] rounded-full border border-white/20 backdrop-blur-sm" style={{
            ...
            borderColor: `${brandColor}40`
                        ...
                        <span className="px-4 py-1.5 bg-black/30 backdrop-blur-sm border border-white/10 rounded-full text-xs font-bold text-white/70 uppercase tracking-widest flex items-center gap-2">
                            <History size={12} /> {resource.is_legacy ? 'Arxiu Històric' : 'Registre Vigent'}
                        </span>
                    </div>

                    <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-[0.9] mb-6 text-white drop-shadow-2xl" style={{
          fontFamily: 'Noto Sans, sans-serif'
              ...
              color: brandColor
                            ...
                            {new Date(resource.created_at || '2024-01-01').toLocaleDateString()}
                        </div>
                        <div className="flex items-center gap-2">
                            <MapPin size={16} />
                            {resource.towns?.name || 'La Torre de les Maçanes'}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div role="region" aria-label="Contingut Principal" className="w-full bg-black relative z-20 -mt-10 px-4 md:px-0">
                <div className="max-w-3xl mx-auto py-16 md:py-24">
                    ...
                    {resource.tags && resource.tags.length > 0 && <div className="hidden md:flex flex-wrap gap-2 mb-12">
                            {resource.tags.map((tag, idx) => <span key={idx} className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg text-xs font-bold text-gray-400 capitalize">
                                    {tag}
                                </span>)}
                        </div>}

                    <article className="resource-biblia-content text-xl md:text-2xl leading-relaxed font-light text-white/90 space-y-8" dangerouslySetInnerHTML={{
          __html: `<p>${formatContentToHtml(resource.display_content)}</p>`
        }} />
                ...
                <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 opacity-50 hover:opacity-100 transition-opacity duration-500">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-xs font-bold uppercase tracking-widest text-[#00F2FF]">Protocol Tabula Rasa v3.0</span>
                            <span className="text-xs font-mono text-gray-500 mt-1">ID: {String(id).toUpperCase()}</span>
                        </div>
                    </div>

                    <div className="flex items-center gap-4">
                         <div className="flex flex-col items-end">
                            <span className="text-xs font-black uppercase mb-2 text-white">Genotip Autenticat</span>
                            <div className="flex gap-2">
                                <span className="w-8 h-1 bg-white/20"></span>
                                <span className="w-16 h-1 bg-white/20"></span>
                                <span className="w-8 h-1" style={{
