...
import { Sparkles, Users, Store, MapPin, Landmark, Building, Link2, Loader2, Search, ChevronRight } from 'lucide-react';
    ...
    return (
        <div className="search-discover-page">
            <SEO
                ...
                description={query ? `Resultats de cerca per a ${query} a Sóc de Poble.Troba gent, entitats i pobles de les nostres comarques.` : 'Descobreix la gent, els pobles i les entitats de la teua comunitat.'}
                keywords={query ? `${query}, cerca, pobles, nostres comarques` : 'pobles, nostres comarques, xarxa social, proximitat'}
            />
            <SearchNavBar
                query={query}
                setQuery={setQuery}
                            ...
                            <div className="semantic-insight-card animate-in">
                                <div className="insight-header">
                                    <div className="hero-avatar small ratoli-glow">
                                        <img src="/uploads/brain/29cb42cf-ba4e-45af-a1f9-254a5b27cd7a/super_ratoli_tia_style_1770057904274.png" alt="Súper Ratolí" />
                                    </div>
                                            ...
                                            <section key="gent" className="result-section">
                                                ...
                                                <div className="results-list">
                                                    {results.gent.map(person => (
                                                        <div key={person.id} className="bg-theme-card rounded-3xl p-3 mb-2 flex items-center gap-3 active:scale-[0.98] transition-transform select-none cursor-pointer shadow-sm hover:shadow-md" onClick={() => navigate(`/gent/${person.id}`)}>
                                                            <Avatar
                                            ...
                                            <section key="pobles" className="result-section">
                                                ...
                                                <div className="results-list">
                                                    {results.pobles.map(town => (
                                                        <div key={town.id} className="bg-theme-card rounded-3xl p-3 mb-2 flex items-center gap-3 active:scale-[0.98] transition-transform select-none cursor-pointer shadow-sm hover:shadow-md" onClick={() => navigate(`/pobles/${town.id}`)}>
                                                            <Avatar
                                                                src={town.image_url}
                                                                role="oficial"
                                                                name={town.name}
                                        ...
                                        <section key={filter.id} className="result-section">
                                            ...
                                            <div className="results-list">
                                                {filteredEntities.map(entity => (
                                                    <div key={entity.id} className={`universal-card result-item-card entity-${entity.type}`} onClick={() => navigate(`/empresa/${entity.id}`)}>
                                                        <div className="card-header clickable">
                                                             <div className="header-left">
                                                                <Avatar
