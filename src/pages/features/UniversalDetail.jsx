...
const UniversalDetail = ({ type: propType, slug: propSlug }) => {
    ...
    useEffect(() => {
        const fetchItem = async () => {
            ...
            try {
                ...
                } else if (type === 'entitat' || type === 'empreses' || type === 'gent' || type === 'grup') {
        ...
        return (
            <div className="flex flex-col items-center justify-center p-8 text-center min-h-[50vh]">
                ...
                <button 
                    ...
                    className="py-4 px-8 rounded-full border border-theme-border text-theme-text font-bold uppercase tracking-widest text-xs hover:bg-black/5 active:scale-95 transition-all"
                >
                    TORNAR ARRERE
                </button>
            </div>
    ...
    return (
        <div className="flex flex-col h-full w-full bg-theme-base animate-in fade-in relative">
            <SEO 
                title={`${itemName} - Sóc de Poble`}
                description={cleanedExcerpt}
                image={itemImage}
                structuredData={schemaData}
                type={type === 'mercat' || type === 'market' ? 'product' : 'article'}
            />
            
            {/* 1. ACTION BAR: BANDA BLAU EXACTA ISO "EL PROJECTE" */}
            <div role="region" aria-label="Capçalera de Secció" className="z-[var(--z-sticky,200)] w-full max-w-full overflow-hidden shadow-lg bg-[#4F46E5] text-white dark:bg-[#F97316] dark:text-[#111111] transition-all shrink-0 touch-manipulation border-b border-black/10 dark:border-white/10">
                <div className="flex items-center justify-between min-h-[56px] px-2 sm:px-4 w-full max-w-7xl mx-auto overflow-hidden">
                    ...
                    <div className="flex items-center justify-start gap-1 shrink-0">
                        ...
                        <button 
                            ...
                            aria-label={t('project.open_index', "Obrir Índex i Pàgines")}
                            title={t('project.open_index', "Obrir Índex i Pàgines")}
                            onClick={() => setIsTocOpen(!isTocOpen)}
                        >
                            <Book size={20} strokeWidth={2.5} />
                            <span className="tabular-nums font-black tracking-widest whitespace-nowrap opacity-90">
                                <span>1</span>/{totalPages}
                            </span>
                        </button>
                    ...
                    <div className="flex items-center justify-end gap-1 sm:gap-2 flex-1 min-w-0 overflow-x-auto no-scrollbar scroll-smooth">
                        <button 
                            ...
                            onClick={() => openTranslationModal({ postId: item?.id || slug, title: itemName })}
                        >
                            {translating ? (
            ...
            <div role="region" aria-label="Contingut Principal" className="flex-1 min-h-0 w-full relative flex flex-col overflow-y-auto overflow-x-hidden custom-scrollbar pb-[max(env(safe-area-inset-bottom),5rem)]">
                ...
                {resolvedImages.length > 0 && (
                    <div className="w-full relative z-0 bg-[var(--bg-panel)] overflow-hidden flex items-center justify-center shrink-0">
                        <UniversalCardMedia 
                            cardVariant={type}
                            mediaList={resolvedImages}
                ...
                <div className="sticky top-0 z-[190] w-full shrink-0 shadow-sm">
                    <UniversalCardHeader
                        item={item}
                        cardVariant={type}
                        displayTown={item?.town_name || item?.town || (item?.seller === 'Sóc de Poble' || item?.author_name === 'Sóc de Poble' ? 'La Torre de les Maçanes' : '')}
                        displayAuthor={item?.seller || item?.author_name || 'Sóc de Poble'}
                        avatarSrc={logoSrc}
                        avatarRole={item?.author_role}
                        isOfficial={item?.official || item?.is_official}
                        displayDate={item?.created_at ? new Date(item.created_at).toLocaleDateString('ca-ES') : ''}
                        displayTime={item?.time || (item?.created_at ? new Date(item.created_at).toLocaleTimeString('ca-ES', { hour: '2-digit', minute: '2-digit' }) : '')}
                        isPageHeader={true}
                    />
                </div>

                {/* 4. CONTINGUT (Títol central, descripció i compra) */}
                <section className="w-full flex flex-col items-center justify-center pb-5 pt-8 bg-[var(--bg-panel)] rounded-b-[2.5rem] shadow-sm mb-0 relative z-10 shrink-0">
                                ...
                                <div className="mt-16 flex flex-col gap-6">
                                    {(item.variations && item.variations.length > 0) ? (
                                        item.variations.map((variation, idx) => {
                                            ...
                                            return (
                                                <div key={idx} className="flex flex-col md:flex-row items-center justify-between p-6 sm:p-8 bg-[var(--bg-panel)] rounded-3xl border border-[var(--border-master)] shadow-sm gap-6 hover:border-[var(--theme-accent-primary)] transition-colors">
                                                    <div className="flex flex-col sm:flex-row items-center sm:items-center text-center sm:text-left gap-4 md:gap-6 w-full md:w-auto">
                                                        {resolvedThumb && (
                                                            <div className="w-24 h-24 sm:w-28 sm:h-28 shrink-0 rounded-2xl overflow-hidden border border-[var(--border-master)] shadow-md bg-black/5 dark:bg-white/5 flex items-center justify-center">
                                                    ...
                                                    <div className="flex flex-col items-center md:items-end shrink-0 w-full md:w-auto mt-4 md:mt-0">
                                                        <span className="text-3xl sm:text-4xl font-black text-[var(--theme-accent-primary)] mb-3">
                                                            {variation.price}
                                                        </span>
                                                        {isVariationOut ? (
                                                            ...
                                                            <button className="w-full md:w-auto px-8 py-4 sm:py-3 bg-[#F97316] text-white font-black uppercase tracking-widest rounded-xl hover:scale-105 active:scale-95 transition-all shadow-[0_0_15px_rgba(249,115,22,0.3)] whitespace-nowrap">
                                                                Afegeix al Cabàs
                                                            </button>
                                                        )}
                                                    </div>
                                        ...
                                        <div className="flex flex-col items-center justify-center p-8 bg-[var(--bg-panel)] rounded-3xl border border-[var(--border-master)] shadow-sm">
                                            <span className="text-4xl font-black text-[var(--theme-accent-primary)] mb-2">
                                            ...
                                            {item.stock_status && (item.stock_status.toLowerCase() === 'esgotat' || item.stock_status.toLowerCase() === 'agotado') ? (
                                                <button disabled className="w-full sm:w-auto px-10 py-4 bg-red-500 text-white font-black uppercase tracking-widest rounded-xl cursor-not-allowed opacity-50 shadow-[0_0_20px_rgba(239,68,68,0.2)]">
                                                    Esgotat
                                                </button>
                                            ) : (
                                                <button className="w-full sm:w-auto px-10 py-4 bg-[#F97316] text-white font-black uppercase tracking-widest rounded-xl hover:scale-105 active:scale-95 transition-all shadow-[0_0_20px_rgba(249,115,22,0.3)]">
                                                    Afegeix al Cabàs
                                                </button>
                                            )}
                                        </div>
            ...
            {!!mediaViewerSrc && (
                <MediaViewerModal 
                    ...
                    imageDescriptions={item?.image_descriptions || []}
                    onNavigate={(newSrc) => setMediaViewerSrc(newSrc)}
                    title={itemName || "Sóc de Poble Visuals"} 
                />
            )}
        </div>
    );
