  ...
  return (
    <div className="flex flex-col w-full bg-[var(--sp-light)] min-h-screen relative text-[var(--sp-void)]">
      ...
      <header className="h-14 flex justify-between items-center px-2 sm:px-4 bg-[var(--sp-void)] text-[var(--sp-light)] shrink-0 border-b border-white/10 sticky top-0 z-40">
        ...
        <h1 className="font-bold text-lg tracking-tight">Legal i Privacitat</h1>
        <div className="w-10 h-10" /> {/* Spacer per centrar el títol */}
      </header>

      {/* Zona principal */}
      <main className="flex-1 flex flex-col w-full z-10 relative">

        {/* 2. HERO (Substituint els fons estranys) */}
        <section className="w-full bg-[var(--sp-accent)] text-[var(--sp-light)] flex flex-col items-center justify-center px-6 py-12 shrink-0 min-h-[220px]">
          <h2 className="text-3xl md:text-5xl font-black tracking-tighter text-center leading-none mb-2">
            Legal, Privacitat i Seguretat
          </h2>
        ...
        {/* 3. METADADES O INFO EXTRA SI CAL */}
        <section className="sticky top-14 z-10 w-full bg-[var(--sp-void)] text-[var(--sp-light)] px-4 py-3 flex items-center gap-3 border-b border-white/10 shrink-0">
          <img
            src="/assets/uploads/brain/llibre_anima_cover_1776032370908.png"
            alt="Sóc de Poble"
        ...
        {/* 4. CONTINGUT NET (Sense UniversalPageLayout ni divs zombis) */}
        <section className="flex-1 p-6 md:p-10 bg-[var(--sp-light)]">
          <div className="max-w-4xl mx-auto w-full text-[var(--sp-void)]">
            <div 
              // Posem els colors explícits per evitar els <p> en text-theme-text gris clar de index.css
              className="w-full max-w-none [&_p]:text-[var(--sp-void)] [&_h3]:text-[var(--sp-void)] text-lg"
              dangerouslySetInnerHTML={{ __html: sanitizeHtml(htmlContent) }} 
            />
            
            {/* TAULER NADIU D'ESTADÍSTIQUES */}
            <div className="mt-12 border-t border-[var(--sp-void)]/10 pt-8">
              <UmamiDashboard />
            </div>
          </div>
        </section>
