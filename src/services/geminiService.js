...
class GeminiService {
  constructor() {
    // Prioritat: Variable d'entorn (Seguretat) > LocalStorage (Sobirania de l'usuari/Demo)
    this.apiKey =
      import.meta.env.VITE_GEMINI_API_KEY ||
      localStorage.getItem("sp_gemini_api_key") ||
      "";
  ...
  async ask(personaKey, query, imageData = null) {
    ...
    if (!this.apiKey) {
      ...
      const isSimulation =
        localStorage.getItem("isPlaygroundMode") === "true" ||
        localStorage.getItem("sb-simulation-mode") === "true" ||
        isLocal;

      if (isSimulation) {
        logger.log(
          "[Gemini] API Key absent, però bateguem en Mode Simulació Master (Local/Playground).",
        );
        ...
        const isGenesis =
          q.includes("genesis") ||
          q.includes("directives") ||
          q.includes("directiva");
        const isIdentity =
          q.includes("identitat") ||
          q.includes("qui ets") ||
          q.includes("qui saps");
      ...
      RATO:
        "Cric-cric... He rastrejat tot el territori. Quan poses la clau, et donaré insights semàntics profunds. Vitaminat!",
          ...
          NANOBANANA:
            "Massa espai buit! Necessitem el Ritu de l'Abundància. Posa la clau i omplirem el bancal de píxels sublims i Zero Radius!",
          FLASH:
            "Ordre rebuda. Executant procés de petició de clau... Velocitat < 0.2s. Posa la API Key al perfil ara mateix. Fet.",
          VIATJANT:
            "He vingut d'Aiora i porte novetats! Però sense la clau del tractor no puc connectar amb les ràdios dels altres pobles. Posa la clau i farem xarxa!",
          REBOST:
            "Tinc el perol al foc però em falta la llenya (API Key). Posa-la al perfil i farem un arròs al forn amb les sobres que flipes!",
          TRELLAT:
            "Veredicte preliminar: Et falta trellat i la clau de l'API. Passa pel perfil i posa-la per a que puga jutjar les teues idees!",
        };
        ...
        return {
          ...
          text:
            ...
            "Santuari de la Saviesa Rural: Falta la clau del tractor per a un bategat complet de Gemini.",
          persona: persona.name,
          avatarName: persona.avatarName,
          type: persona.type,
          is_mock: true,
      ...
      return {
        error: true,
        message:
          "Xe, falta la clau del tractor! Necessites configurar la teua API Key de Gemini per parlar amb l'expert.",
      };
    }
    ...
    try {
      ...
      const text =
        data.candidates?.[0]?.content?.parts?.[0]?.text || "No hi ha resposta.";
  ...
  /**
   * Helper per centralitzar respostes de simulació
   */
