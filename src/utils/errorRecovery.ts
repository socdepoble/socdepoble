...
export const safeAsync = async <T,>(fn: () => Promise<T>, fallback: T, retries = 2, baseDelay = 800): Promise<T> => {
...
export const withRetry = async <T,>(fn: () => Promise<T>, retries = 3, baseDelay = 1000): Promise<T> => {
  for (let i = 0; i <= retries; i++) {
    try {
      return await fn();
    } catch (e) {
